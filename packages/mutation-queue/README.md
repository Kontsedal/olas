...
A `serial` run that waits behind another is written to storage when you call `run(...)`, not when its turn comes. If the first request hangs or backs off and the tab reloads, every run queued behind it is still on disk, and the next load replays them in the order you called them. Each queued run gets an entry of its own, even when `dedupeBy` gives it the key of the run ahead.

The queue writes a run's entry before its first request goes out. On an async store like IndexedDB, a supersede, `reset()` or dispose can land while that write is pending. The request then stays unsent, even from a `mutate` that ignores its `signal`.

When the root starts, the plugin lists every entry under `keyPrefix` and checks each one (see [Stored entries](#stored-entries)). It groups them by `id`, sorts each group by the monotonic `seq` with a fallback to `enqueuedAt`, waits until the tab is online, then replays serially per group. An entry that stays on disk ends its group's pass: a failure worth a retry, or a run executing it here or in another tab. The group's later entries wait for the next pass, so none of them reaches the server ahead of it. Two tabs that open in the same millisecond can give unrelated entries the same `seq`, and `runId` breaks that tie. So every tab replays the same entries in the same order, whatever order its storage lists them in. Different `id` buckets run in parallel. A replay pass also runs on every reconnect, so an in-session failure retries on reconnect and not only on reload. `ctx.inject(MutationQueue).replayNow()` starts one by hand. All three paths funnel through one guarded runner, which prevents overlapping replays, wrapped in the cross-tab lock described below. After each successful replay, `onReplaySettle` fires so the app can invalidate affected queries.
...
- **Same-`id` runs are serial across loads.** `order/create` followed by `order/cancel` for the same id always run in order. A replay that fails with a retryable error holds back the rest of its `id` until the next pass. Use distinct `id`s for orthogonal operations.
...
- **One entry per logical operation, per tab.** A manual retry of a failed run supersedes the entry that run left behind. A replay pass skips runs executing right now, in this tab or another. Both rules are described in [Retries and the one-entry rule](#retries-and-the-one-entry-rule).
...
**The entry holds the newest write.** An autosave with `latest-wins` and a `dedupeBy` on the document id is the common case. Each new draft supersedes the run in flight and collapses onto its entry. Once the superseded run settles, the entry takes the newest draft's variables, so a reload or a dispose replays that draft and not the stale one. A run that collapses onto an entry kept for a replay, after a dispose or a retryable failure, rewrites it with its own variables before its request goes out. A success drops the entry only when no newer run's variables ride on it. When a replay of the entry, or the run that wrote it, succeeds while a newer draft is still sending, the entry stays and holds that draft. A `serial` run waiting behind another never collapses, because the run ahead settles before it starts and would take the entry with it.

**A replay never touches a run that is executing.** A reconnect or a `replayNow()` that lands between a run's start and its settle sees that run's entry on disk and would fire the same request again. The queue skips those entries; the live run's own settle disposes of them. A `serial` run still waiting its turn counts as executing, so the live queue sends it, not the replay. Another tab's runs count too: see [Cross-tab replay coordination](#cross-tab-replay-coordination).
...
The replay lock covers replays, not live runs, so each live run also marks its entry as its tab's until it settles. With Web Locks the mark is a lock named for the entry, and without them a `localStorage` lease that a heartbeat refreshes every 15 seconds. A replay pass in another tab skips a marked entry and the rest of its group. A closed tab's locks go with it, and a lease its tab stops refreshing expires after 30 seconds, so a dead tab's entries replay on a later pass. Two windows stay open. A Web Lock is granted a moment after the run starts, and a replay that reads the entry in that moment sends it too. A run that starts in another tab after a replay began sending the same entry goes out as well.
...
`result` is `success`, `retry-later`, `max-attempts`, `not-retryable` or `aborted`. `reason` is `in-flight`, `in-other-tab`, `waiting`, `not-registered`, `not-persisted`, `max-attempts` or `ttl-expired`. `in-other-tab` is an entry a run in another tab has marked, and `waiting` is an entry left behind one that stays on disk. A payload carries ids and counts, and no variables. Its shape serves the panel, and a minor release can change it.
...
- **Cross-tab coordination is best-effort.** Web Locks (when available) gives an exclusive replay lock and marks each live run's entry; the `localStorage`-lease fallback narrows but doesn't fully close the double-replay window. Server-side dedupe (`idempotencyKey`) is the authoritative gate.
