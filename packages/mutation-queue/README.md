...
**The entry holds the newest write.** An autosave with `latest-wins` and a `dedupeBy` on the document id is the common case. Each new draft supersedes the run in flight and collapses onto its entry. Once the superseded run settles, the entry takes the newest draft's variables, so a reload or a dispose replays that draft and not the stale one. A run that collapses onto an entry kept for a replay, after a dispose or a retryable failure, rewrites it with its own variables before its request goes out. A `serial` run waiting behind another never collapses, because the run ahead settles before it starts and would take the entry with it.
