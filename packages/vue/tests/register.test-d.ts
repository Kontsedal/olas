// Type-level test: augmenting `Register` types `useRoot()` without a generic.
...
export function typedSetup(): void {
  const api = useRoot()
  ...
  const other = useRoot<{ other: boolean }>()
