 ...
 * ```
 *
 * Was `ctx.use(...)`. Needs a query engine on the root:
 ...
 * ```
 *
 * Was `ctx.cache(...)`. Unlike `createQuery` this needs **no** query engine:
 ...
 * ```
 *
 * Was `ctx.mutation(...)`. Needs a query engine: mutations participate in the
