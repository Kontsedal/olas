...
**Async data is shared per-root through `QueryClient`.** `defineQuery` produces a module-scoped value branded `'query'` or `'infiniteQuery'`. The query value carries a `__clients: Set<QueryClient>` so unbound query operations can detect ambiguous roots. Use `bindQuery(ctx, query)` or `root.bindQuery(query)` to select one root explicitly. `createQuery(ctx, query, keyFn)` binds a subscription through the controller's root client. See `entities/query-client.md` and `flows/query-subscription.md`.
