...
The packages build ESM only (`decisions/esm-only-build.md`). CI (`.github/workflows/ci.yml`) runs install → build → typecheck → lint → `check:peer-ranges` → `check:peer-bumps` → `check:doc-snippets` → `test:coverage` → the examples' tests → publint → attw → `smoke:dist` → `check:public-types` → `api:check` → size. A second job builds on Node 22 and runs the dist smoke on Node 20.19, 22 and 24 (`decisions/toolchain.md`). On 2026-09-25 the root `pnpm test` ran 2,232 tests across 176 files, including the cross-package `packages/integration` suite. The example apps ship their own controller-level tests on top. See `../CLAUDE.md` for the full command list.
