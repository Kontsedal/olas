...
`setData(updater, { track: false })` is a **canonical cache write** — cross-tab receive (`client.applyRemoteSetData`), entity backprop and realtime patches (`client.setEntryData`). It writes `data` but pushes NO snapshot and does NOT flip `hasPendingMutations`, returning a no-op `Snapshot`. This is why a fire-and-forget plugin write can no longer wedge `hasPendingMutations` at `true` — the T1.1 bug was exactly that those callers went through the tracked path and discarded the returned snapshot. `InfiniteEntry.setData` mirrors the same `track` option. Pinned by `regressions.test.ts` (R-Q1.1).
