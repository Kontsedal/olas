...
`hydrate` and `hydrateLive` apply entries through `eachHydrationEntry`, which skips an entry that fails `isHydrationEntry` or throws, and warns in development. A key nested deep enough to overflow the recursive `stableHash`, or an entry of `null`, used to make `createRoot({ hydrate })` throw. `acceptsState` also requires `entries` to be an array. Pinned by `regressions.test.ts`, "a malformed hydration payload".
