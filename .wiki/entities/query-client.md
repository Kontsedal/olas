...
The fields are at `packages/core/src/query/client.ts:827-846`. Regular and infinite queries differ enough, one `data` against an array of `pages` with a paging mechanic, that a unified entry would be more confusing than two parallel paths. They share the `AsyncState` shape through `Entry` and `InfiniteEntry`'s common signals.

`byId` holds the queries this root has bound, by `id` (`client.ts:835-841`). `index()` fills it on the first `bindEntry`, `bindInfiniteEntry` or `bindQuery` of a query, and warns in development when two different query values share an id in one root (`client.ts:904-917`). Plugins, hydration payloads and devtools address queries through it. A duplicate id is therefore a collision within one app, never across the process. Pinned by `plugin-host.test.ts`, "two queries sharing an id in one root warn in dev".

## Root-wide query defaults

`client.defaults: QueryDefaults` holds `queryEngine({ defaults })`, always an object and never `undefined`, so call sites stay a flat `??` chain (`client.ts:861-867`, `client.ts:883`). Spec §5.9.

Every consumer resolves the same way, **`spec.X ?? client.defaults.X ?? built-in`**, so an explicit per-query field wins:

| Field | Resolved in |
|---|---|
| `staleTime`, `retry`, `retryDelay`, `networkMode`, `structuralShare` | `ClientEntry` constructor → `Entry` options (`client.ts:378-382`); `InfiniteClientEntry` constructor → `InfiniteEntry` (`client.ts:647-651`) |
| `gcTime` | `ClientEntry` and `InfiniteClientEntry` fields (`client.ts:359`, `client.ts:628`) |
...
| `refetchOnWindowFocus`, `refetchOnReconnect` | folded into `client.refetchOnWindowFocus` and `client.refetchOnReconnect` at construction (`client.ts:884-885`) |
...
`Entry<T>` is the state machine. `ClientEntry<T>` wraps it and adds the **per-root** parts (`client.ts:304-584`):

- `subscriberCount` — `acquire()` increments it and `release()` decrements it. The 0→1 and 1→0 transitions call `emitActivity`, which reports `onActivate` and `onDeactivate` to plugins (`client.ts:395-443`). It counts every hold, a prefetch in flight included, because it drives gc.
...
- `gcTimer` — started on `release()` when the count hits zero, and by `scheduleGcIfOrphan()` for an entry a non-subscribing path created, such as `prefetch`, `setData` or `invalidate`. `acquire()` clears it, and it fires `client.dropEntry(this)`. Since 0.9 it holds a **cancellation closure** from `scheduleExpiry` in `expiry-timer.ts` rather than a raw handle. So `gcTime: Infinity` schedules nothing and keeps the entry for the life of the root, and a `gcTime` past 2,147,483,647 ms is chunked instead of overflowing. Before that, both clamped to about 1 ms and collected almost at once, the opposite of the setting. The staleness timer shares the helper; see `../pitfalls/isstale-needs-timer.md` and `packages/core/tests/expiry-timers.test.ts`.
  - A `release()` on a closing client arms no timer, because it returns on `client.isClosing` (1.0, `client.ts:431-433`). `root.dispose()` calls `queryClient.close()` before disposing the controllers, whose subscriptions then release entries the client disposes next.
  - A `release()` after the entry was disposed is a no-op (1.0, `client.ts:420`). A `prefetch` still in flight at `root.dispose()` releases in its `finally`. That release used to arm a fresh gc timer for a dead entry, which held a Node process open for `gcTime`.
- `intervalTimer` — a self-rescheduling `scheduleExpiry` chain, not a `setInterval` (`client.ts:454-517`). Each tick **re-arms first**, then runs two guards. It skips while `document.visibilityState === 'hidden'`. It also skips while a fetch is in flight, per `isFetching.peek()`, so the tick joins the running fetch rather than aborting it. A fetch slower than the interval therefore cannot livelock (T3.2). For a fetch parked for the network, per `isPaused.peek()`, it starts nothing and calls `entry.resumeParked()`, which runs the park only while `navigator.onLine` does not read false. An offline tick therefore parks no new waiter, and a park the `online` event missed still ends on the next tick (1.0, second pass). Before, the tick skipped a parked entry outright, and one with no `window` or an early `online` event stayed parked. Re-arming before the guards matters twice. It keeps the cadence a metronome, measuring gap N+1 from tick N rather than from whenever the fetch settles. It also stops a skipped tick from ending the chain. Pinned in `regressions.test.ts` under R-Q3.2 and in `query-focus-online.test.ts`'s "refetchInterval — hidden tab".
- `nextIntervalMs` — the closure that resolves the gap before the next tick, `undefined` when the query declared no `refetchInterval` (`client.ts:326-336`, `client.ts:360-364`). `refetchInterval` is `number | ((data: T | undefined) => number)` per spec §5.9.
...
- `unsubFocus` and `unsubOnline` — `window` focus and `online` subscriptions, installed on the 0→1 acquire when the resolved flag is `true` (`client.ts:404-409`). The resolution is `spec.refetchOnWindowFocus ?? client.refetchOnWindowFocus`, and the same for reconnect, where the client's value is `defaults.refetchOnWindowFocus ?? false` (`client.ts:884-885`). They are cleared on release to zero and on dispose. The handler skips the refetch when `entry.isStaleNow()` is false, so a query fetched within `staleTime` ignores the event, and it joins a fetch already in flight (`client.ts:557-572`). For a parked entry it calls `entry.resumeParked()` instead, whatever the staleness, and starts no fetch of its own. The entry's reconnect listener drains through the same method, so whichever fires first runs the one request, and the other finds nothing parked. Starting a fetch here as well used to make one `online` event start a request and then abort it for a second. Pinned by `query-focus-online.test.ts`, "offline and reconnect scheduling".
...
`invalidate`, `invalidateAll`, `invalidateInfinite`, `invalidateAllInfinite` and `host.queries.invalidate` all route the entry through `invalidateEntry` (`client.ts:1579-1615`):
...
The entry catches up the same way when data lands and leaves the stale mark standing. That data is a response requested before a subscriber-less `markStale()`, a page, or a row stamped before it. It does so only while someone holds it, which the entry asks through the `hasSubscribers` option both entry kinds pass (`client.ts:387`, `client.ts:657`). That is how a subscriber, a `resume()` or a `prefetch` that joined the older fetch still gets the refetch §5.7 promises (1.0, second pass).

## Mutation in-flight counter

`mutationsInflight$: Signal<number>` lives on the client, not on each mutation (`client.ts:846`). `MutationImpl` receives it and increments it when a run starts, after `onMutate`, and decrements it in `finally`. A queued `serial` run counts too. `waitForIdle()` waits for this counter and for every entry's `isFetching` (`client.ts:1384-1438`). It re-checks up to 100 times, because a settling fetch can start another, then throws an error listing the entries still fetching. `root.waitForIdle()` wraps it in a second loop that also waits for plugin work (`packages/core/src/controller/root.ts:214-231`).

## SSR

`dehydrate()` walks `maps` and `infiniteMaps` and emits `{ id: query.__id, key: keyArgs, data, lastUpdatedAt }` for each entry that holds data, plus `pageParams` for an infinite entry (`client.ts:1350-1382`). An entry at `status: 'success'` with no data still ships, as before. It skips an entry with no data that is pending, errored or idle. Spec §15. Until the second 1.0 pass it kept only `status: 'success'`, and `status` reads `'pending'` over data during a background refetch and `'error'` after a failed one, so a dehydrate at those moments dropped an entry holding data. `lastUpdatedAt` is the public signal, the time the shipped data was written. Pinned by `ssr.test.ts`, "an entry holding data mid-refetch is serialized".

Two methods take a payload, and both check `version === 1` first (`acceptsState`, `client.ts:1160-1172`):
- `hydrate(state)` is the `RootOptions.hydrate` path, called from the constructor (`client.ts:887`). It only buffers, into `hydratedData: Map<hydrationKey, HydratedSlot>` (`client.ts:1273-1284`).
- `hydrateLive(state, origin?)` backs `root.hydrate` and `host.queries.hydrate` (`client.ts:1267-1270`). `applyDehydratedEntry` writes through to an entry already bound, superseding its fetch, and buffers the rest (`client.ts:1214-1244`). A row stamped before the bound entry's server data is skipped, because `applyHydration` returns `false`, and the client then reports no write. An optimistic write does not count toward that stamp (`entry.md`, "`serverUpdatedAt` is not `lastUpdatedAt`").
...
`bindEntry` and `bindInfiniteEntry` check `hydratedData` on the first bind of a key and pass the values into the new entry's `initialData` and `initialUpdatedAt` (`client.ts:1482-1487`, `client.ts:1850-1859`). An infinite entry adopts only a payload whose `pageParams` line up with its pages (`infinitePayload`, `client.ts:199-206`). A buffered slot is consumed once, and later binds refetch as usual. Either path reports one `'hydrate'` write to plugins, carrying the origin of whoever hydrated. See `../flows/ssr.md`.

## Plugins: the engine half of the host

`QueryClient implements PluginEngine` (`client.ts:826`). It holds the root's `PluginSet` in `plugins`, or `null` when the root has no plugins (`client.ts:870`, `client.ts:886`). `PluginSet.install` receives the client as its engine, so a root without an engine never bundles this module. The client supplies four things:

- **Emit sites.** `emitWrite`, `emitInfiniteWrite`, `emitInvalidated`, `emitRemoved` and `emitActivity` (`client.ts:919-982`). Each returns at once when no plugin listens for its hook. `refOf` caches one `QueryRef` per query (`client.ts:891-902`).
- **Middleware.** `runFetch` (`client.ts:985-989`) and `mutationLifecycle(origin?)` (`client.ts:995-1005`). The second returns `undefined` when no plugin observes mutations, so the runner builds no events.
- **`queryHost(origin)`** (`client.ts:1008-1051`). `get`, `keys` and `peek` read through `byId`. `write` and `replace` go through `writeByKey`, a canonical patch or whole-record write that does nothing when this root holds no entry for the key (`client.ts:1115-1158`). `invalidate`, `hydrate` and `dehydrate` wrap the client's own methods, and `hashKey` is `stableHash`. Every write and invalidation carries `origin`.
- **`mutationHost(origin)`** (`client.ts:1054-1095`). `has` and `get` read the global `defineMutation` registry. `run` keeps one runner per plugin and id, built with `mutationLifecycle(origin)`, so its events carry the plugin's name. It rejects after dispose or for an id nothing registered.
...
`hydrate` and `hydrateLive` apply entries through `eachHydrationEntry`, which skips an entry that fails `isHydrationEntry` or throws, and warns in development (`client.ts:1292-1304`, `client.ts:184-193`). A key nested deep enough to overflow the recursive `stableHash`, or an entry of `null`, used to make `createRoot({ hydrate })` throw. `acceptsState` also requires `entries` to be an array. Pinned by `regressions.test.ts`, "a malformed hydration payload".
