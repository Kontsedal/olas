...
Callbacks on a detached run execute after the controller is torn down, so they must stay at client level (`query.invalidate()`, a toast). If the root is gone the run still completes but its cache writes no-op, since `QueryClient.dispose()` deregisters the client from every query (`client.ts:1979-2000`).

`reset()` is similar but doesn't mark disposed — it aborts inflight, drains the queue, and clears `data`/`error`/`lastVariables`/`isPending` and sets `status` back to `'idle'`. The mutation remains usable. `mutation.ts:686-712`; spec §6.2 lists it among the abort triggers; pinned by `mutation.test.ts:57` and regression B2.
