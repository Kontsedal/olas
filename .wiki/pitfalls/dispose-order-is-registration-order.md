...
type: pitfall
covers:
  - packages/core/src/controller/instance.ts:276-324
...
`ControllerInstance.dispose()` walks **one** list — every `ctx.*` primitive, every child, every hook, in creation order — backwards, dispatching on `entry.kind` (`instance.ts:276-297`). Nothing groups by kind. So the relative order of an effect and an `onDispose` hook is decided entirely by which was created first:
