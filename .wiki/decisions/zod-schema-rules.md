...
- A path that ends at a `Form` or a `FieldArray`, or names a key the schema lacks, keeps its path. Core's router (`routeFormIssues`, `packages/core/src/forms/form.ts:194-223`) puts it in that node's `topLevelErrors`, or in the root's when nothing resolves.
