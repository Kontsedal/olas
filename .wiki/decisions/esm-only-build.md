...
The adapter review later that day added about 0.9 kB brotli to react, 0.08 kB to vue, 0.09 kB to svelte and 0.12 kB to devtools. React's share is the stream placement: a byte tokenizer with an element stack, the batches folded into a boundary's first root, the per-root stream cursor, and the release of a root on an `<Activity>` hide (`../pitfalls/stream-chunks-split-tags.md`, `../pitfalls/effect-cleanup-not-unmount.md`). Those numbers were measured without a build, as esbuild-minified brotli of each package's source against the same of `HEAD`, and added to the last `size-limit` readings. The budgets went to 5.2 KB for react and 0.97 KB each for vue and svelte, about 5% over; devtools stays inside 19.7 KB.

The third review round (2026-09-25 evening) fixed 47 findings across every runtime package, and a build then measured them with `size-limit`. Core reads 5.67, 10.66, 19.02 and 25.98 kB. The largest costs are the optimistic-layer replay and commit fold (`../pitfalls/visible-data-is-not-a-baseline.md`), the offline park handling, the baseline copies in forms (`copyPlainData`) and the collection's per-item error isolation. Cross-tab reads 1.93 kB for relayed write sources and mirrored guesses, mutation-queue 4.03 kB for per-run cross-tab leases and the group stop, entities 2.71 kB, `createPersisted` 1.31 kB and `persistQueryCachePlugin` 1.3 kB. React reads 4.83 kB, vue 0.91 kB and svelte 0.91 kB, below the estimates above. Twelve budgets were re-set to about 5% over these readings: 5.95, 11.2, 20 and 27.3 KB for core, 5.1 KB for react, 0.96 KB each for vue and svelte, 2.05 KB for cross-tab, 2.85 KB for entities, 4.25 KB for mutation-queue, 1.38 KB for `createPersisted` and 1.37 KB for `persistQueryCachePlugin`.
