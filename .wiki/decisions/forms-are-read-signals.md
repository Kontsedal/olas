...
The same pass made `Form.submit` resolve a `SubmitResult<R>` (`packages/core/src/forms/form-types.ts:146-149`). `ok: true` carries `data`. `ok: false` carries a `reason`: `'invalid'`, `'error'` (with `error`), `'busy'` or `'disposed'`. Before, `{ ok: boolean; data?; error? }` could not tell "invalid" from "already submitting" without matching an error message.
