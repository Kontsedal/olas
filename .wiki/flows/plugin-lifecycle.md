...
covers:
  ...
  - packages/core/src/query/client.ts:304-371
  - packages/core/src/query/client.ts:794-1061
  - packages/core/src/query/client.ts:1117-1169
  - packages/core/src/query/client.ts:1328-1408
  - packages/core/src/query/client.ts:1436-1495
  - packages/core/src/query/client.ts:1594-1711
  - packages/core/src/query/client.ts:1965-2001
  - packages/core/src/query/actions.ts:24-69
  - packages/core/src/query/bind.ts:145-169
...
- The `QueryClient` is created first (`root.ts:36-43`), so a plugin's `setup` can reach the cache. The client buffers `RootOptions.hydrate` there and reports nothing yet (`packages/core/src/query/client.ts:790`, `client.ts:1158-1169`).
...
Every cache write reaches the plugins through one private emitter, `QueryClient.emitWrite` (`client.ts:822-842`). It asks `plugins.listens('onWrite')` first, so a root whose plugins have no `onWrite` builds no event. `updatedAt` is the entry's `lastUpdatedAt`, or `Date.now()` when that is absent. An infinite write goes through `emitInfiniteWrite` first (`client.ts:845-861`), which adds the entry's `pageParams`.

| `source` | Emitted by | `origin` |
|---|---|---|
| `'fetch'` | the `onFetched` closure `bindEntry` hands the entry (`client.ts:1370-1378`), on each successful fetch | `undefined` |
| `'hydrate'` | `bindEntry` adopting a buffered payload (`client.ts:1398-1408`), or `applyDehydratedEntry` on a bound entry (`client.ts:1117-1145`) | whoever hydrated: `undefined` for `RootOptions.hydrate` and `root.hydrate`, the plugin's name for `host.queries.hydrate` |
| `'optimistic'` | `setData` (`client.ts:1669-1688`) | the handle's origin |
| `'rollback'` | that snapshot's `rollback()`, only when the data changed (`client.ts:1692-1708`) | the handle's origin |
| `'write'`, `'replace'` | `writeData` and `replaceData` (`client.ts:1594-1667`), or `host.queries.write` and `replace` through `writeByKey` (`client.ts:1024-1061`) | the handle's origin, or the plugin's name |

`createQueryActions(query, getClient, origin)` passes the handle's origin to every write and invalidation (`packages/core/src/query/actions.ts:24-69`). The module-level query methods and `bindQuery(ctx, q)` pass none. `bindQuery(ctx, q, { origin })` and `root.bindQuery(q, { origin })` pass theirs (`client.ts:1328-1347`). `refOf` caches one `QueryRef` per query for every event (`client.ts:794-805`).

A `host.queries` write differs from an app write in one respect. `writeByKey` returns without writing when this root holds no entry for the key (`client.ts:1032-1033`), where the app's `write` binds one.
...
`onInvalidate` takes the same route. `invalidate`, `invalidateAll` and `host.queries.invalidate` first run `invalidateEntry`, which refetches a subscribed entry or marks an unsubscribed one stale (`client.ts:1459-1495`). They then call `emitInvalidated` with the caller's origin (`client.ts:863-871`). The host's `invalidate` sends the devtools `cache:invalidated` first, as the app-side ones do (1.0).
...
`ClientEntry.acquire` reports `onActivate` on the 0→1 subscriber transition, and `release` reports `onDeactivate` on 1→0 (`client.ts:321-371`). `InfiniteClientEntry` does the same (`client.ts:574-650`). A second subscriber joining or leaving reports nothing.

`dropEntry` and `dropInfiniteEntry` report `onRemove` with `reason: 'gc'` when gc drops an entry (`client.ts:1436-1450`, `client.ts:1776-1790`). A root dispose reports no removals: the client disposes its entries directly, and delivery is closed by then.
...
- **Fetch.** Both entry kinds call their fetcher through `client.runFetch` (`client.ts:304-308`, `client.ts:551-556`). `runFetch` calls the fetcher directly when no plugin wraps fetches (`client.ts:888-892`). The retry loop sits inside the entry, so each attempt runs through the chain with its own `attempt` number. `FetchContext` carries the `QueryRef`, the key, the call args, the signal, the attempt and, for an infinite query, the `pageParam` (`packages/core/src/plugin/types.ts:248-258`).
...
`QueryClient.mutationLifecycle(origin?)` returns `undefined` unless a plugin set `observesMutations` (`client.ts:898-908`). `createMutation` reads it when a controller builds the mutation (`packages/core/src/query/bind.ts:156-166`), so a root whose plugins ignore mutations builds no mutation events.
...
`host.mutations.run(id, vars)` runs a `defineMutation` through the same runner (`client.ts:968-998`). The client keeps one runner per plugin and id, so `serial` and `latest-wins` hold across that plugin's runs. The runner gets `mutationLifecycle(origin)`, so its events and its `MutateContext` carry the plugin's name. It counts toward `mutationsInflight$`, and so toward `waitForIdle`.
...
2. `queryClient.close()`. An entry released from here arms no gc timer (`client.ts:361`, `client.ts:1965-1971`).
...
5. `queryClient.dispose()`. The client disposes its entries, then the runners `host.mutations.run` created (`client.ts:1973-2001`). A replay still in flight is cancelled here, after its plugin has disposed.
