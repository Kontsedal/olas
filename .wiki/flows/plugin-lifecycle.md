...
covers:
  ...
  - packages/core/src/plugin/types.ts:289-361
  - packages/core/src/controller/root.ts:20-142
  - packages/core/src/controller/root.ts:206-223
  - packages/core/src/query/client.ts:306-375
  - packages/core/src/query/client.ts:799-1067
  - packages/core/src/query/client.ts:1123-1175
  - packages/core/src/query/client.ts:1326-1440
  - packages/core/src/query/client.ts:1442-1501
  - packages/core/src/query/client.ts:1600-1717
  - packages/core/src/query/client.ts:1966-2007
  - packages/core/src/query/actions.ts:24-69
  - packages/core/src/query/bind.ts:137-193
  - packages/core/src/query/mutation.ts:304-373
  - packages/core/src/query/mutation.ts:476-673
edges:
...
createRoot(app, { queries: queryEngine(), plugins: [a, b] })
  ...
  3. plugin scopes seeded, then RootOptions.scopes            root.ts:67-85
  4. the root factory runs                                    root.ts:90-97
...
- The `QueryClient` is created first (`root.ts:36-43`), so a plugin's `setup` can reach the cache. The client buffers `RootOptions.hydrate` there and reports nothing yet (`packages/core/src/query/client.ts:797`, `client.ts:1164-1175`).
...
- `host.provide` works only while that plugin's `setup` runs, and a later call throws (`host.ts:97-106`). `createRoot` seeds the collected bindings on the root instance, then seeds `RootOptions.scopes` over them (`root.ts:78-85`), so a test's fake wins.
...
- The root factory runs last. A factory throw disposes the plugins, then the client (`root.ts:90-97`).
...
Every cache write reaches the plugins through one private emitter, `QueryClient.emitWrite` (`client.ts:828-848`). It asks `plugins.listens('onWrite')` first, so a root whose plugins have no `onWrite` builds no event. `updatedAt` is the entry's `lastUpdatedAt`, or `Date.now()` when that is absent. An infinite write goes through `emitInfiniteWrite` first (`client.ts:850-867`), which adds the entry's `pageParams`.

| `source` | Emitted by | `origin` |
|---|---|---|
| `'fetch'` | the `onFetched` closure `bindEntry` hands the entry (`client.ts:1376-1384`), on each successful fetch | `undefined` |
| `'hydrate'` | `bindEntry` adopting a buffered payload (`client.ts:1400-1414`), or `applyDehydratedEntry` on a bound entry (`client.ts:1123-1151`) | whoever hydrated: `undefined` for `RootOptions.hydrate` and `root.hydrate`, the plugin's name for `host.queries.hydrate` |
| `'optimistic'` | `setData` (`client.ts:1675-1694`) | the handle's origin |
| `'rollback'` | that snapshot's `rollback()`, only when the data changed (`client.ts:1695-1716`) | the handle's origin |
| `'write'`, `'replace'` | `writeData` and `replaceData` (`client.ts:1600-1673`), or `host.queries.write` and `replace` through `writeByKey` (`client.ts:1024-1067`) | the handle's origin, or the plugin's name |

`createQueryActions(query, getClient, origin)` passes the handle's origin to every write and invalidation (`packages/core/src/query/actions.ts:24-69`). The module-level query methods and `bindQuery(ctx, q)` pass none. `bindQuery(ctx, q, { origin })` and `root.bindQuery(q, { origin })` pass theirs (`client.ts:1326-1353`). `refOf` caches one `QueryRef` per query for every event (`client.ts:800-811`).

A `host.queries` write differs from an app write in one respect. `writeByKey` returns without writing when `entryByKey` finds no entry for the key in this root (`client.ts:1038-1039`), where the app's `write` binds one.
...
`onInvalidate` takes the same route. `invalidate`, `invalidateAll` and `host.queries.invalidate` first run `invalidateEntry`, which refetches a subscribed entry or marks an unsubscribed one stale (`client.ts:1465-1501`). They then call `emitInvalidated` with the caller's origin (`client.ts:869-877`). The host's `invalidate` sends the devtools `cache:invalidated` first, as the app-side ones do (1.0).
...
`ClientEntry.acquire` reports `onActivate` on the 0→1 subscriber transition, and `release` reports `onDeactivate` on 1→0 (`client.ts:327-375`). `InfiniteClientEntry` does the same (`client.ts:578-656`). A second subscriber joining or leaving reports nothing.

`dropEntry` and `dropInfiniteEntry` report `onRemove` with `reason: 'gc'` when gc drops an entry (`client.ts:1442-1456`, `client.ts:1782-1796`). A root dispose reports no removals: the client disposes its entries directly, and delivery is closed by then.
...
- **Fetch.** Both entry kinds call their fetcher through `client.runFetch` (`client.ts:307-310`, `client.ts:556-560`). `runFetch` calls the fetcher directly when no plugin wraps fetches (`client.ts:894-898`). The retry loop sits inside the entry, so each attempt runs through the chain with its own `attempt` number. `FetchContext` carries the `QueryRef`, the key, the call args, the signal, the attempt and, for an infinite query, the `pageParam` (`packages/core/src/plugin/types.ts:290-306`).
- **Mutate.** `MutationImpl.runWithRetry` wraps each `mutate` call when the lifecycle carries a `wrap` (`packages/core/src/query/mutation.ts:644-673`). `MutateContext` carries the `MutationRef`, the `runId`, the variables, the signal, the attempt and the origin.
...
`QueryClient.mutationLifecycle(origin?)` returns `undefined` unless a plugin set `observesMutations` (`client.ts:904-914`). `createMutation` reads it when a controller builds the mutation (`packages/core/src/query/bind.ts:169-193`), so a root whose plugins ignore mutations builds no mutation events.

`MutationImpl.report` sends each step of a run (`mutation.ts:354-372`):
- `'start'` after `onMutate` returns and before the first `mutate` call (`mutation.ts:534`). A run aborted during `onMutate` reports nothing.
- Then exactly one of `'success'`, `'error'` or `'cancel'` (`mutation.ts:536-607`). A run whose work finished before a late abort reports `'success'`, and the `settledOutcome` flag keeps it to that one outcome (`mutation.ts:538-560`).
...
`host.mutations.run(id, vars)` runs a `defineMutation` through the same runner (`client.ts:974-1004`). The client keeps one runner per plugin and id, so `serial` and `latest-wins` hold across that plugin's runs. The runner gets `mutationLifecycle(origin)`, so its events and its `MutateContext` carry the plugin's name. It counts toward `mutationsInflight$`, and so toward `waitForIdle`.
...
5. `queryClient.dispose()`. The client disposes its entries, then the runners `host.mutations.run` created, held in `pluginMutations` (`client.ts:1979-2007`). A replay still in flight is cancelled here, after its plugin has disposed.
