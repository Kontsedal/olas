...
covers:
  ...
  - packages/core/src/plugin/types.ts:289-409
  - packages/core/src/controller/root.ts:20-148
  - packages/core/src/controller/root.ts:217-234
  - packages/core/src/query/client.ts:397-467
  - packages/core/src/query/client.ts:914-1213
  - packages/core/src/query/client.ts:1269-1348
  - packages/core/src/query/client.ts:1508-1626
  - packages/core/src/query/client.ts:1628-1687
  - packages/core/src/query/client.ts:1786-1927
  - packages/core/src/query/client.ts:2235-2276
...
- The `QueryClient` is created first (`root.ts:36-43`), so a plugin's `setup` can reach the cache. The client buffers `RootOptions.hydrate` there and reports nothing yet (`packages/core/src/query/client.ts:911`, `client.ts:1337-1348`).
...
Every cache write reaches the plugins through one private emitter, `QueryClient.emitWrite` (`client.ts:943-971`). It asks `plugins.listens('onWrite')` first, so a root whose plugins have no `onWrite` builds no event. `updatedAt` is the entry's `lastUpdatedAt`, or `Date.now()` when that is absent. A `'commit'` passes the server clock instead (below). An infinite write goes through `emitInfiniteWrite` first (`client.ts:977-995`), which adds the entry's `pageParams`. Every call site passes the entry it wrote. `emitWrite` reads its `serverState()` into the event's `server` through `serverOf` (`client.ts:124-139`): the data `dehydrate()` would ship, beneath any live optimistic layer, stamped with the server clock. It is read only when a plugin listens.

### `server`, and why a plugin needs it (third 1.0 review)

While an optimistic layer is live, `data` is the guess on screen, whatever the source is. A `'write'`, a `'replace'` or an infinite page `'fetch'` made under a guess carried it, so the query-cache persister, which kept canonical sources only, stored the guess. The correction came as a `'rollback'`, which it skipped. The core agent's BACKLOG idea named two fixes: report the last settle as canonical, or give the event the data beneath the guesses. The event now carries it, because a relay needs it too. Cross-tab sends it with a write made under a guess, so the peer writes server truth and shows the rest as a guess (`../modules/cross-tab.md`). `server` is absent only when the entry holds neither data nor a server answer, such as an optimistic create into an entry nothing fetched. Pinned by the "onWrite — the server truth under a guess" group in `plugin-host.test.ts`.

### `host.queries.setData`: a plugin's own guess (third 1.0 review)

`queryHost` gives plugins `setData(id, key, updater, options?)` (`client.ts:1072-1084`). It is the app's `setData` body, `optimisticWrite` for a regular entry and `optimisticInfiniteWrite` for an infinite one (`client.ts:1879`, `client.ts:2150`), on an entry found by id and key. It returns the `Snapshot`, and `undefined` for a key the root holds no entry for. The events carry the plugin's name as `origin`, and the settle reports `'rollback'` or `'commit'` as an app's does. Cross-tab shows a peer's guess through it, so the guess stays a guess in the receiving tab. Pinned by the "host.queries.setData" group in `plugin-host.test.ts`.

| `source` | Emitted by | `origin` |
|---|---|---|
| `'fetch'` | the `onFetched` closure `bindEntry` hands the entry (`client.ts:1558-1566`), on each successful fetch | `undefined` |
| `'hydrate'` | `bindEntry` adopting a buffered payload (`client.ts:1584-1600`), or `applyDehydratedEntry` on a bound entry (`client.ts:1269-1324`) | whoever hydrated: `undefined` for `RootOptions.hydrate` and `root.hydrate`, the plugin's name for `host.queries.hydrate` |
| `'optimistic'` | `setData` (`client.ts:1865-1897`), or `host.queries.setData` | the handle's origin, or the plugin's name |
| `'rollback'` | that snapshot's `rollback()`, only when the data changed (`client.ts:1898-1927`) | the handle's origin |
| `'commit'` | `emitCommit` (`client.ts:1934-1947`), when a settle of that snapshot leaves no live layer on the entry and a layer committed: its own `finalize()`, or the last settle after a commit made under another layer, a `rollback()` included, which then reports no `'rollback'` | the settling handle's origin |
| `'write'`, `'replace'` | `writeData` and `replaceData` (`client.ts:1786-1863`), or `host.queries.write` and `replace` through `writeByKey` (`client.ts:1164-1213`) | the handle's origin, or the plugin's name |

`createQueryActions(query, getClient, origin)` passes the handle's origin to every write and invalidation (`packages/core/src/query/actions.ts:24-69`). The module-level query methods and `bindQuery(ctx, q)` pass none. `bindQuery(ctx, q, { origin })` and `root.bindQuery(q, { origin })` pass theirs (`client.ts:1508-1535`). `refOf` caches one `QueryRef` per query for every event (`client.ts:915-926`).

A `host.queries` write differs from an app write in one respect. `writeByKey` returns without writing when `entryByKey` finds no entry for the key in this root (`client.ts:1178-1179`), where the app's `write` binds one.
...
`onInvalidate` takes the same route. `invalidate`, `invalidateAll` and `host.queries.invalidate` first run `invalidateEntry`, which refetches a subscribed entry or marks an unsubscribed one stale (`client.ts:1651-1687`). They then call `emitInvalidated` with the caller's origin (`client.ts:997-1005`). The host's `invalidate` sends the devtools `cache:invalidated` first, as the app-side ones do (1.0).
...
- "onWrite — commit": one `'commit'` for a finalize with no other live layer, a commit under another layer reported by the last settle, its `updatedAt` on the server clock, and the devtools `cache:set-data`;
...
`ClientEntry.acquire` reports `onActivate` on the 0→1 subscriber transition, and `release` reports `onDeactivate` on 1→0 (`client.ts:419-467`). `InfiniteClientEntry` does the same (`client.ts:685-766`). A second subscriber joining or leaving reports nothing.

`dropEntry` and `dropInfiniteEntry` report `onRemove` with `reason: 'gc'` when gc drops an entry (`client.ts:1628-1642`, `client.ts:2013-2027`). A root dispose reports no removals: the client disposes its entries directly, and delivery is closed by then.
...
- **Fetch.** Both entry kinds call their fetcher through `client.runFetch` (`client.ts:398-401`, `client.ts:662-666`). `runFetch` calls the fetcher directly when no plugin wraps fetches (`client.ts:1022-1026`). The retry loop sits inside the entry, so each attempt runs through the chain with its own `attempt` number. `FetchContext` carries the `QueryRef`, the key, the call args, the signal, the attempt and, for an infinite query, the `pageParam` (`packages/core/src/plugin/types.ts:353-369`).
...
`QueryClient.mutationLifecycle(origin?)` returns `undefined` unless a plugin set `observesMutations` (`client.ts:1032-1042`). `createMutation` reads it when a controller builds the mutation (`packages/core/src/query/bind.ts:172-202`), so a root whose plugins ignore mutations builds no mutation events.
...
`host.mutations.run(id, vars)` runs a `defineMutation` through the same runner (`client.ts:1114-1144`). The client keeps one runner per plugin and id, so `serial` and `latest-wins` hold across that plugin's runs. The runner gets `mutationLifecycle(origin)`, so its events and its `MutateContext` carry the plugin's name. It counts toward `mutationsInflight$`, and so toward `waitForIdle`.
...
2. `queryClient.close()`. An entry released from here arms no gc timer (`client.ts:457`, `client.ts:2235-2242`).
...
5. `queryClient.dispose()`. The client disposes its entries, then the runners `host.mutations.run` created, held in `pluginMutations` (`client.ts:2248-2276`). A replay still in flight is cancelled here, after its plugin has disposed.
