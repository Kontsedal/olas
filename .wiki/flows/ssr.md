...
```ts
{
  version: 1,
  entries: [
    { id: queryIdentity, key: keyArgs, data, lastUpdatedAt },
    ...
  ]
}
```
...
`QueryClient.hydrate` populates `hydratedData: Map<hydrationKey, { data, lastUpdatedAt }>`, where `hydrationKey = JSON.stringify([id, keyHash])` in `client.ts`. It is namespaced by query identity and NOT by hash alone, so a colliding-key query B cannot adopt query A's payload. That is T1.2, pinned by `regressions.test.ts` under R-Q1.2. **No entries are created at hydrate time.** The hydrated row is consumed lazily on the first `bindEntry` whose `hydrationKey(query.__id, hash)` matches. The new `Entry` is constructed with `initialData` + `initialUpdatedAt` set, which puts it in `status: 'success'` from the start. A hand-authored payload (server building `DehydratedState` directly) must set each entry's `id` to the target query's `queryId`.
