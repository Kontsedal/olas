...
**One transport listener per `RealtimeService`.** `onReconnect` builds on `createConnectionState`, so a controller with both used to open two `onConnectionChange` subscriptions, and N controllers opened N. Now `joinConnection` (`index.ts:394-434`) keeps a hub per service in a `WeakMap`:
...
- `last` holds the latest report while the subscription is open. A listener that joins an open subscription starts from it, because the transport's synchronous "current state" call, if it makes one, went to the first listener only. A listener that joins before any report is set to `'connected'`: `listener(hub.last ?? 'connected')` (`index.ts:425`).
...
- **A resume is a fresh start (2026-09-25 review).** A suspended controller leaves the hub, and its signal keeps the state from before the suspend. The join used to set the signal only when `last` was defined. A transport that reports only changes, and came back while the hub was closed, reports nothing on the new subscribe, so the signal stayed `'offline'` after resume and `onReconnect` never fired. The join now resets it to `'connected'`, the same optimism a new `createConnectionState` starts with. That move from `'offline'` counts as a reconnect: `onReconnect`'s `prev` survives the suspend, and the controller's patchers were unsubscribed during it, so the refetch is due. A transport that reports its state on subscribe sets `last` before the listener joins, so it gets no flash of `'connected'`. The cost: a change-only transport still offline after the gap reads `'connected'` until its next report, as it does on a first start. Pinned by the two resume cases in `shared-connection.test.ts`.
