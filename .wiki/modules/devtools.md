...
covers:
  ...
  - packages/core/src/query/mutation.ts:384-405
  - packages/core/src/controller/root.ts:224-227
  - packages/core/src/controller/instance.ts:386-451
  - packages/core/src/controller/instance.ts:554-574
  - packages/core/src/plugin/host.ts:166-169
  - packages/core/src/query/client.ts:206-224
...
Lifecycle events from `ControllerInstance` go straight through `rootShared.devtools.emit(...)`; see `instance.ts:257-267, 308-310, 381-383, 437-450`. Each call site is wrapped in `if (__DEV__)`, so production builds elide it.

**Cache events** (Phase 13; extended T8.1 and W10). `QueryClient` holds a `devtools?: DevtoolsEmitter`. `devtoolsEntryEvents(devtools, queryId, queryKey)` (`client.ts:152-204`) builds one `EntryEvents` callback bundle, and both `ClientEntry` and `InfiniteClientEntry` pass it to their entry (`client.ts:318`, `client.ts:573`):
...
Every other cache write emits `cache:set-data` through the private `emitDevtoolsSetData` helper (`client.ts:1083-1104`). It uses the plugins' `WriteSource` vocabulary, and each call site passes its source:
...
Every event of a failing optimistic mutation shares one `causeId`, and `devtools-events.test.ts` pins that membership. Read from `mutation.ts:476-612`, the order is:
