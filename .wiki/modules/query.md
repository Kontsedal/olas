...
covers:
  ...
  - packages/core/src/controller/root.ts:120-264
edges:
...
| `local.ts` | `LocalCacheImpl` and `createLocalCache(fetcher, options, deps)`. Backs `createCache`. It carries the three writes a `Query` has: `setData` (optimistic), and `write` and `replace` (canonical, 1.0). `replace` calls `Entry.supersedeByWrite(true)`, because the owning controller is the cache's one subscriber for as long as it exists. The key effect compares keys by `stableHash`, as `createQuery` does, so a thunk that re-runs to an equal key refetches nothing (1.0, fourth pass; `cache.test.ts`, "ctx.cache — an equal key does nothing"). `createCache` in `bind.ts` registers the cache with `ctxInternals.trackLocalCache`, so `root.waitForIdle()` counts its fetches. |
...
| `client.ts` | `QueryClient`, `ClientEntry<T>`, `InfiniteClientEntry`. The per-root entry registry, `gcTime`, the refetch-interval chain (`resolveRefetchInterval` at `client.ts:69-100`, `armIntervalTick` at `client.ts:491-534`), `mutationsInflight$`, dehydrate, hydrate and `waitForIdle`. The query half of the plugin host (below). |
...
| `infinite.ts` | `InfiniteEntry<TPage, TItem, PageParam>`, the paginated variant. Owns `pages`, `pageParams`, `fetchNextPage` and `fetchPreviousPage`. A refetch from an interval, an invalidate or `refetch()` re-fetches every loaded page through `runRefetchAll`, not page one alone (T3.7). It starts from the first loaded page's param, so a list paged backwards refetches as it is (1.0, fourth pass). SSR, focus and reconnect refetch, `offlineFirst`, plugins and devtools all cover it (`../decisions/infinite-query-parity.md`). |
...
A `Query` is module-scoped. Binding a handle or entry registers its client in `query.__clients`, and indexes the query in the client's `byId` map (`client.ts:928-941`). Bound actions select that client; unbound calls fail when more than one client is registered. Disposal unregisters the client. See `../decisions/per-root-query-client.md` and `query-isolation.test.ts`.

`invalidate` and `invalidateAll` return a `Promise<void>` for the selected root. Fetch errors route to that root's `onError` as `kind: 'cache'` with `queryId`, `key`, and the `attempt` and `cause` the entry recorded (`client.ts:1651-1687`). Ambiguity and root disposal reject. A subscriber-less entry is marked stale without refetching. When a `replace` discards the refetch, the entry fetches once more and the promise follows that catch-up (`entities/entry.md`). See spec §5.7, §6.4 and §21.5.
...
- **`peek(...keyArgs): T | undefined`** (`client.peekData`, `client.ts:1758-1764`) is a synchronous read. It looks the entry up in `maps` **without** `bindEntry`, so a peek cannot mint the entry it asks about. It reads through `.peek()`, so it registers no reactive dependency. `undefined` conflates "no entry" with "not settled", deliberately: the caller that cares is guarding a write, and both answers mean *don't*. The bound handle reads only its selected root. An unbound peek returns undefined for zero clients and throws on multiple clients.
- **`write(...keyArgs, updater): void`** (`client.writeData`, `client.ts:1786-1820`) is a canonical patch: `Entry.setData(updater, { track: false })`, so no snapshot record and no `hasPendingMutations` flip. It re-runs the updater on each live snapshot's baseline and leaves a fetch in flight alone (`../pitfalls/visible-data-is-not-a-baseline.md`). It binds the entry when absent, as `setData` does. Plugins and devtools see `source: 'write'`.
- **`replace(...keyArgs, value): void`** (`client.replaceData`, `client.ts:1842-1863`) is a canonical whole record. It writes like `write`, then supersedes a fetch in flight through `Entry.supersedeByWrite` when `value` is defined. If the discarded fetch was an invalidation's, the entry fetches once more to reconcile (spec §6.4). The source is `'replace'`.
- **`prefetch(...keyArgs): Promise<T>`** (`client.prefetch`, `client.ts:2205-2233`) holds the entry until its fetch settles. It joins a fetch in flight through `Entry.settled()` rather than `firstValue()`, which would resolve with the stale data the refetch replaces. A cancel that leaves no data rejects it with an `AbortError`, and it releases the entry; before, it waited forever and the entry was never gc'd. `prefetchInfinite` (`client.ts:2181-2204`) mirrors it. Pinned by `prefetch-settles.test.ts`.
- **`setData(...keyArgs, updater): Snapshot`** (`client.setData`, `client.ts:1865-1927`) is the **optimistic** write, reported as `'optimistic'`. Its snapshot is the caller's to settle, and a rollback that changes the data reports a `'rollback'` write. A settle that leaves no live layer after a commit reports `'commit'` (`emitCommit`, `client.ts:1934-1947`). See `pitfalls/no-invalidator-still-refetches.md` for the `cancel()`-first rule that applies to it even when nothing invalidates the query.
...
- `onError(err, vars, snapshot)` typically calls `snapshot?.rollback()`, and the runner rolls back after `onError` anyway. On success the runner calls `finalize()` (`mutation.ts:639-643`), which folds the committed layer into the baselines of the layers still live below it. Rolling back the top-of-stack snapshot restores its captured pre-value. A non-top rollback under concurrency chain-splices instead and replays the layers above it, so the failed delta leaves the screen at once and out-of-order rollbacks still converge on the bottom baseline, commits included (spec §6.4, `entities/entry.md`).
...
`QuerySpec.networkMode` (`'online'` default, `'always'` and `'offlineFirst'`) gates fetches on `navigator.onLine`. `online` defers a fetch requested while offline in `Entry.scheduleDeferredFetch` (`entry.ts:412-428`), after `startFetch` cancels the fetch in flight in the same batch (`entry.ts:354-361`). The entry resumes on the `online` event: `subscribeReconnect` in `focus-online.ts` calls `Entry.drainDeferred` (`entry.ts:442-456`). A fetch made online before that event ends the park too, and `cancel()` drops it (1.0, fourth pass; `../entities/entry.md`). `offlineFirst` runs the fetch, but a `fetch` `TypeError` raised while offline parks and retries on reconnect instead of surfacing the error; see the `runWithRetry` catch in `entry.ts:530-545` (T3.5). Both parked paths set the `isPaused` signal on `AsyncState`. It is `true` while waiting for the network and `false` whenever a fetch is in flight or settled. `always` never parks. Spec §5.5; pinned by `query-focus-online.test.ts` (R-Q3.5). Infinite entries park the same way in both fetch loops, `runRefetchAll` and `runFetch`, through `parksOnOffline` and `park` (`infinite.ts:1508-1528`). The drain re-runs the parked direction, and a parked refetch re-runs the whole refetch. An interval tick and a focus or reconnect refetch start no fetch of their own for a parked entry (`triggerEventRefetch`, `client.ts:581-596`). They call `resumeParked` instead, which drains only while `navigator.onLine` does not read false. One `online` event therefore makes one request, and a park the event missed ends on the next trigger. A worker has no `window` to fire that event (1.0, second pass; see `../entities/entry.md`).

## SSR

`root.dehydrate()` calls `QueryClient.dehydrate` (`client.ts:1422-1450`). It walks `maps` and `infiniteMaps` and emits `{ id, key, data, lastUpdatedAt }` for each entry's `serverState()`, plus `pageParams` for an infinite entry. An entry mid-refetch or after a failed refetch still ships its data. Under a live optimistic write the row carries the data beneath the guess, and the stamp is the server clock, which a guess and its rollback do not move (1.0, fourth pass). An entry with no data and no server answer is skipped.

`createRoot(def, { hydrate: state })` buffers the payload into the client's `hydratedData` map through `QueryClient.hydrate` (`client.ts:1337-1348`), keyed by query id and key hash. The first `bindEntry` matching both consumes the row and threads it into the new `Entry` as `initialData` (`client.ts:1537-1555`, `client.ts:407`). That site also reports one `WriteEvent` with `source: 'hydrate'` (`client.ts:1584-1600`). Without it, a plugin observing every write, such as entities, would miss every hydrated row, because `Entry.applySuccess` never runs for an entry that starts with `initialData`. `bindInfiniteEntry` does the same for a payload with aligned `pageParams` (`client.ts:1962-2008`).

`root.hydrate(state)` and `host.queries.hydrate(state)` apply a payload to a running root through `hydrateLive` (`client.ts:1331-1334`), which checks the version and calls `applyDehydratedEntry` per row (`client.ts:1269-1324`). A bound entry takes the row through `Entry.applyHydration`, which supersedes a fetch in flight, and reports one `'hydrate'` write. It skips a row stamped before the server data the entry holds, and an optimistic write does not count toward that stamp. An unbound key buffers into `hydratedData`, which keeps the newest row per key. The React streaming hydrator feeds `root.hydrate` batch by batch. See `flows/ssr.md`.
...
- **`queryHost(origin)`** (`client.ts:1045-1100`) is `host.queries`. It addresses entries by query `id` and key through `byId`. `write` and `replace` go through `writeByKey` (`client.ts:1164-1213`), a canonical write that is a no-op when the root holds no entry for the key. A `replace` there supersedes a fetch in flight only when it left the entry holding data, for a regular and an infinite entry alike: the app-side rule (1.0). Before, an infinite `replace` with empty pages cancelled too. `invalidate` sends the devtools `cache:invalidated`, as the app-side `invalidate` does.
- **`mutationHost(origin)`** (`client.ts:1103-1112`) is `host.mutations`: `has`, `get` and `run` over the mutation registry. `run` keeps one runner per plugin and id (`client.ts:1114-1144`).
- **Emitters.** `emitWrite`, `emitInfiniteWrite`, `emitInvalidated`, `emitRemoved` and `emitActivity` (`client.ts:943-1019`) build events only when a plugin listens for them.
- **Middleware.** `runFetch` (`client.ts:1022-1026`) sends each fetch attempt through `wrapFetch`, and `mutationLifecycle` (`client.ts:1032-1042`) hands the runner `onMutation` and `wrapMutate`.
