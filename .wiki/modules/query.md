...
| `bind.ts` | The ctx-taking entry points: `createQuery`, `createCache`, `createMutation`, `bindQuery` (`bind.ts:26-230`). Each reaches the controller through `ctxInternals`. |
...
| `client.ts` | `QueryClient`, `ClientEntry<T>`, `InfiniteClientEntry`. The per-root entry registry, `gcTime`, the refetch-interval chain (`resolveRefetchInterval` at `client.ts:62-93`, `armIntervalTick` at `client.ts:428-466`), `mutationsInflight$`, dehydrate, hydrate and `waitForIdle`. The query half of the plugin host (below). |
...
A `Query` is module-scoped. Binding a handle or entry registers its client in `query.__clients`, and indexes the query in the client's `byId` map (`client.ts:846-859`). Bound actions select that client; unbound calls fail when more than one client is registered. Disposal unregisters the client. See `../decisions/per-root-query-client.md` and `query-isolation.test.ts`.

`invalidate` and `invalidateAll` return a `Promise<void>` for the selected root. Fetch errors route to that root's `onError` as `kind: 'cache'` with `queryId`, `key`, and the `attempt` and `cause` the entry recorded (`client.ts:1500-1536`). Ambiguity and root disposal reject. A subscriber-less entry is marked stale without refetching. When a `replace` discards the refetch, the entry fetches once more and the promise follows that catch-up (`entities/entry.md`). See spec §5.7, §6.4 and §21.5.
...
- `onError(err, vars, snapshot)` typically calls `snapshot?.rollback()`, and the runner rolls back after `onError` anyway. On success the runner calls `finalize()` (`mutation.ts:628-632`). Rolling back the top-of-stack snapshot restores its captured pre-value. A non-top rollback under concurrency chain-splices instead, so out-of-order rollbacks still converge on the original value (spec §6.4, `entities/entry.md`).
- For `concurrency: 'latest-wins'`, the previous run's snapshot rolls back **synchronously before the new run's `onMutate` is called** (`mutation.ts:462-471`). See `pitfalls/latest-wins-rollback-order.md`.
...
`QuerySpec.networkMode` (`'online'` default, `'always'` and `'offlineFirst'`) gates fetches on `navigator.onLine`. `online` defers a fetch requested while offline in `Entry.scheduleDeferredFetch` (`entry.ts:337-353`), after `startFetch` cancels the fetch in flight in the same batch (`entry.ts:284-291`). The entry resumes on the `online` event: `subscribeReconnect` in `focus-online.ts` calls `Entry.drainDeferred` (`entry.ts:355-374`). `offlineFirst` runs the fetch, but a `fetch` `TypeError` raised while offline parks and retries on reconnect instead of surfacing the error; see the `runWithRetry` catch in `entry.ts:411-426` (T3.5). Both parked paths set the `isPaused` signal on `AsyncState`. It is `true` while waiting for the network and `false` whenever a fetch is in flight or settled. `always` never parks. Spec §5.5; pinned by `query-focus-online.test.ts` (R-Q3.5). Infinite entries park the same way in both fetch loops, `runRefetchAll` and `runFetch`, through `parksOnOffline` and `park` (`infinite.ts:1146-1166`). The drain re-runs the parked direction, and a parked refetch re-runs the whole refetch. An interval tick and a focus or reconnect refetch skip a parked entry (`triggerEventRefetch`, `client.ts:513-523`), so the drain owns reconnect: one `online` event makes one request.

## SSR

`root.dehydrate()` calls `QueryClient.dehydrate` (`client.ts:1276-1303`). It walks `maps` and `infiniteMaps` and emits `{ id, key, data, lastUpdatedAt }` for each entry in `status: 'success'`, plus `pageParams` for an infinite entry. Error and idle entries are skipped.

`createRoot(def, { hydrate: state })` buffers the payload into the client's `hydratedData` map through `QueryClient.hydrate` (`client.ts:1199-1210`), keyed by query id and key hash. The first `bindEntry` matching both consumes the row and threads it into the new `Entry` as `initialData` (`client.ts:1390-1408`, `client.ts:345`). That site also reports one `WriteEvent` with `source: 'hydrate'` (`client.ts:1435-1449`). Without it, a plugin observing every write, such as entities, would miss every hydrated row, because `Entry.applySuccess` never runs for an entry that starts with `initialData`. `bindInfiniteEntry` does the same for a payload with aligned `pageParams` (`client.ts:1767-1812`).

`root.hydrate(state)` and `host.queries.hydrate(state)` apply a payload to a running root through `hydrateLive` (`client.ts:1193-1196`), which checks the version and calls `applyDehydratedEntry` per row (`client.ts:1156-1186`). A bound entry takes the row through `Entry.applyHydration`, which supersedes a fetch in flight, and reports one `'hydrate'` write. An unbound key buffers into `hydratedData`. The React streaming hydrator feeds `root.hydrate` batch by batch. See `flows/ssr.md`.
...
- **`queryHost(origin)`** (`client.ts:950-993`) is `host.queries`. It addresses entries by query `id` and key through `byId`. `write` and `replace` go through `writeByKey` (`client.ts:1057-1100`), a canonical write that is a no-op when the root holds no entry for the key. A `replace` there supersedes a fetch in flight only when it left the entry holding data, for a regular and an infinite entry alike: the app-side rule (1.0). Before, an infinite `replace` with empty pages cancelled too. `invalidate` sends the devtools `cache:invalidated`, as the app-side `invalidate` does.
- **`mutationHost(origin)`** (`client.ts:996-1005`) is `host.mutations`: `has`, `get` and `run` over the mutation registry. `run` keeps one runner per plugin and id (`client.ts:1007-1037`).
- **Emitters.** `emitWrite`, `emitInfiniteWrite`, `emitInvalidated`, `emitRemoved` and `emitActivity` (`client.ts:861-924`) build events only when a plugin listens for them.
- **Middleware.** `runFetch` (`client.ts:927-931`) sends each fetch attempt through `wrapFetch`, and `mutationLifecycle` (`client.ts:937-947`) hands the runner `onMutation` and `wrapMutate`.
