...
`QuerySpec.networkMode` (`'online'` default, `'always'` and `'offlineFirst'`) gates fetches on `navigator.onLine`. `online` defers a fetch requested while offline in `Entry.scheduleDeferredFetch` (`entry.ts:354-370`), after `startFetch` cancels the fetch in flight in the same batch (`entry.ts:301-308`). The entry resumes on the `online` event: `subscribeReconnect` in `focus-online.ts` calls `Entry.drainDeferred` (`entry.ts:384-403`). `offlineFirst` runs the fetch, but a `fetch` `TypeError` raised while offline parks and retries on reconnect instead of surfacing the error; see the `runWithRetry` catch in `entry.ts:440-455` (T3.5). Both parked paths set the `isPaused` signal on `AsyncState`. It is `true` while waiting for the network and `false` whenever a fetch is in flight or settled. `always` never parks. Spec §5.5; pinned by `query-focus-online.test.ts` (R-Q3.5). Infinite entries park the same way in both fetch loops, `runRefetchAll` and `runFetch`, through `parksOnOffline` and `park` (`infinite.ts:1187-1207`). The drain re-runs the parked direction, and a parked refetch re-runs the whole refetch. An interval tick and a focus or reconnect refetch start no fetch of their own for a parked entry (`triggerEventRefetch`, `client.ts:557-572`). They call `resumeParked` instead, which drains only while `navigator.onLine` does not read false. One `online` event therefore makes one request, and a park the event missed ends on the next trigger. A worker has no `window` to fire that event (1.0, second pass; see `../entities/entry.md`).
