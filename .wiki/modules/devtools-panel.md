...
edges:
  ...
  - { type: tested-by, target: ../../packages/devtools/tests/store-reattach.test.tsx }
...
**The build minifies the stylesheet.** `tsdown.config.ts` runs `scripts/minify-css.ts` over the `DEVTOOLS_CSS` literal. The source keeps its comments, which carry the ui-rules reasoning, and the published bundle drops them. A JS minifier cannot reach inside a template literal, and the comments cost 1.4 KB brotli. The plugin returns a `magic-string` sourcemap for the rewrite, because rolldown 1.2 warns on a transform without one. `tests/minify-css.test.ts` checks that the minified sheet keeps every rule and class, and that the map is not empty.
...
**Attaching again replays nothing twice (2026-09-25 review).** `usePanelStore` keeps its store in a `useMemo`, and StrictMode's effect replay attaches it, detaches it and attaches it again. Each attach got the bus's replay, and `pushTimeline` logged it each time: two controllers showed four `controller:constructed` rows. `attach` now sets `replaying` around `root.debug.subscribe`, which delivers the replay synchronously (`store.ts:556-586`). While it is set, `repeatsTree` (`store.ts:642-656`) passes over a replayed `controller:constructed` for a cell the store has seen constructed and not disposed, and the `controller:suspended` that follows one for a cell it already had suspended. Such an event still updates the tree, through `route`, and puts no row on the timeline. The check reads the cell's `constructed` flag, not its presence: a replay delivers a child before its parent, and the child's event creates the parent's cell as a bare ancestor. A controller that changed while the store was detached still gets its row, such as one suspended meanwhile. Pinned by `store-reattach.test.tsx`, at the store and through the panel under `<StrictMode>`.
