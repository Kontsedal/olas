...
`version: N` wraps writes in `{"$olas":1,"v":N,"d":<serialized>}` (`encodeForStorage`, `packages/persist/src/index.ts:410-416`). `decode` (`index.ts:426-431`) turns a stored string into `[payload, fromVersion]`, and `applyLoaded` and `applyRemote` both start from it. A matching `v` deserializes directly. An older `v` goes to `migrate(raw, fromVersion)`, and so does a legacy un-enveloped payload, which arrives as `fromVersion: undefined`. A versioned reader with no migrator deserializes a legacy raw payload as is. The migrator returns the upgraded `T`, re-persisted as an envelope under `needsRewrite`, or `undefined` to drop the entry and leave the source at its default. A throwing migrator routes `onError('migrate')`. Version mismatch with no migrator discards the stored value, and so does a newer `v` (see "A newer build's payload" below). Tested in `persist.test.ts` (T6.1f).

### The rewrite yields to a peer's change (second 1.0 review)

With async storage, a cross-tab change that arrives during the load waits in `pendingRemoteRaw`, and `settleReady` applies it after the loaded value. The `needsRewrite` block then wrote the migrated loaded value over it. Storage held the migrated old value while the source showed the peer's, and the IndexedDB adapter broadcast the stale write back to the peers. `applyLoaded` now reads `lastChange` before `settleReady` and skips the rewrite when `applyRemote` bumped it (`index.ts:645-649`). The skip also covers a buffered change `applyRemote` ignores or is still migrating, because storage holds that peer's payload either way. Pinned by "a peer change that raced the load is not overwritten by the rewrite" in `coverage-persisted.test.ts`, which failed on the old code.

### A newer build's payload (second 1.0 review)

`isNewer` (`index.ts:436-437`) marks a payload whose version is above this reader's. Since the first review the cross-tab path migrated a peer of any other version. A v2 tab that received a v3 payload passed it to a step migrator such as `while (v < 2)`, which returned it unchanged, and the tab held the v3 shape. `migrate` brings values forward only, as SPEC §13.4 says, so both paths now drop a newer payload. `applyRemote` returns before `migrate` (`index.ts:542`). In `applyLoaded`, `isNewer(from)` skips the migrate branch (`index.ts:601`), so the payload is discarded, and storage keeps it for the build that wrote it. The load path had a worse form of the bug. It rewrote the unchanged v3 value under a v2 envelope, so the newer build would then migrate v3 data as if it were v2 data. The tab keeps its default, and its next write stores a v2 value, which the newer build migrates forward. Pinned by "a payload from a newer build never reaches the migrator" and "a payload from a newer build is not migrated at load, and storage keeps it", both in `coverage-persisted.test.ts`, and both failed on the old code.
...
- **The map starts from storage, with or without `restore`.** A flush writes the whole map, so an entry missing from it is deleted from storage. `setup` always calls `read`, and `load` (`packages/persist/src/query-cache.ts:263-279`) seeds the map with every stored entry the filters keep; only with `restore` does it also hydrate. `flush` writes only once `seeded` is set (`query-cache.ts:228-233`). While a read is in flight it sets `held` and returns, and the read's `finally` flushes once it lands.
- **A read that fails holds the writes.** `seed` sets `seeded` for any read that returned, a corrupt payload included, since writing over garbage loses nothing (`query-cache.ts:282-289`). A rejected or throwing `storage.get` leaves `seeded` false and reports `'restore'`. The next flush calls `read` again before it writes, so the plugin makes at most one read per flush, and dispose flushes a held write. A session whose reads all fail writes nothing.
- **An opted-out query's stored entries go.** `kept` (`query-cache.ts:214-217`) drops a stored entry whose query the root has used, through `queries.get`, and `include` rejects. `load` applies it when seeding and `flush` when writing. An entry of a query the root has not used is kept, since nothing says it opted out.
