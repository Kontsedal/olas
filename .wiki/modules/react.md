...
edges:
  ...
  - { type: related, target: ../pitfalls/effect-cleanup-not-unmount.md }
  - { type: related, target: ../pitfalls/suspend-on-undefined-data.md }
...
function useSuspenseQuery<T>(sub): UseSuspenseQueryResult<T>   // useQuery(sub, { suspense: true }); throws sub.firstValue()
    // until the first load settles, even on undefined; a disabled query suspends until enabled + loaded (dev warns)
...
The listing matches `packages/react/src/index.ts:1-47`. `useRoot` is at `context.ts:82-88` and `HydrationBoundary` at `context.ts:532-612`.
...
- **Suspense tracks `data` and `status` itself**, whatever the component reads, because the suspend decision reads them. The decision is "the first load has not settled": no success, no data and no `lastUpdatedAt` (`suspendUntilData`, `hooks.ts:315-332`). It was `data === undefined`, which looped forever on a load that settled on `undefined`, because `firstValue()` resolves at once then (`../pitfalls/suspend-on-undefined-data.md`). The decision cannot change without `data` or `status` moving, so `lastUpdatedAt` is not tracked for it.
...
- **Render acquires, commit claims.** A render with no committed root for its `def` calls `acquireRoot` (`context.ts:326-382`). It returns the unclaimed root an earlier attempt of the same element built, found in `uncommittedByProps` by the props object, or builds one through `buildRoot` and records it in `uncommitted`. The first layout effect calls `claimRoot`, which takes the root out of both maps, and stores it in `ownedRef` (`context.ts:568-579`). A render reads `ownedRef`, and only the commit writes it.
- **Why the props object.** A retry of a discarded render reuses the element, and so the props object. A retry that built a new root refetched whatever the child suspended on, and the child suspended again, forever. An element the parent re-created is a new props object, so `findReusable` looks further: an unclaimed root built from the same `def`, the same `hydrate` object and `deps` with the same members (`sameDeps`). That covers the common shape, a component below an outer `<Suspense>` that renders the boundary with inline options. Two boundaries sharing a `def` can both find one root this way; the commit that fails to claim it rebuilds. A root that takes the stream matches only another that does. When the options changed between attempts, no root matches, the retry builds a new one, and a development build warns once (`warnedRebuild`). Pinned by "(n) a parent that re-creates the boundary below an outer Suspense loads once", "(o) two boundaries that share a def and options get two roots" and "(p) a retry whose options changed warns once, naming the fix".
- **The sweep.** `armSweep` disposes a root that stays unclaimed for `ORPHAN_GRACE_MS` (ten seconds) after `root.waitForIdle()` resolves (`context.ts:388-408`). The countdown starts at idle because a child suspended on the root's own fetch is retried when that fetch settles, and the retry must still find the root. `ORPHAN_MAX_MS` (a minute) bounds the wait: a root with a hung fetch never goes idle, and it is disposed anyway. Each reuse restarts both timers. Pinned by "(m) an unclaimed root that never goes idle is disposed after a minute".
- **The server builds a root per render.** `acquireRoot` returns a fresh root when there is no `window`, and records nothing. Nothing commits on the server, so no root there could be claimed, and reuse by props object would hand one request's root to the next when an element is hoisted to module scope. Pinned by `coverage-server-env.test.tsx`, "an element hoisted to module scope builds a root per server render".
- **A failed claim rebuilds.** `claimRoot` returns `false` when the root is gone: the sweep disposed it, or another fiber rendering the same element claimed it first. The effect then builds a fresh root from the same `def` and options and calls `forceRender()`, before paint. The same path rebuilds a root whose release expired while the boundary was hidden. The rebuilt root reuses the same options, and so the same `queryEngine()` value, which works because an engine is a definition. Pinned by "(b2) StrictMode with a query engine and a hydrate payload" and "(k) one element rendered twice gets two independent roots".
- **A cleanup releases the root, in its own passive effect.** A `useEffect` with `[]` deps calls `releaseOwned` in its cleanup and `reclaimOwned` when it runs (`context.ts:589-600`). React runs that cleanup on an unmount, on an `<Activity>` hide and in StrictMode's replay, with no way to tell them apart. So the release suspends the root and disposes it after `RELEASE_GRACE_MS`, a minute, unless the effect runs again first; StrictMode's remount and an `<Activity>` show take the root back and resume it. A hide longer than the grace period expires the root, and the claim effect rebuilds it on show. The cost: an unmount disposes a minute late, with the root suspended meanwhile. `../pitfalls/effect-cleanup-not-unmount.md` has the reasoning. It must not be a layout effect: React runs layout-effect cleanups when a `<Suspense>` above hides content it already showed, and that hide keeps the root running. A layout-effect dispose killed the app root whenever a later suspension showed the fallback. The claim effect has no cleanup, because a cleanup there would also run when `root` changes. Pinned by "(a) unmount suspends the root, and disposes it once the grace period ends", "(l) a Suspense above that hides the committed boundary keeps its root" and the "HydrationBoundary under <Activity>" block.
...
- **A new `def` never disposes in render.** The render acquires a root for the new `def` with `hydrate` dropped from the owned root's options, and with no stream (`context.ts:553-560`). The server payload and the streamed batches described the first root's tree, so the replacement starts from its own fetches and takes no streamed row, earlier or later. The claim effect disposes the old root when the new one commits. A transition that suspends therefore keeps the committed tree on a live root. Pinned by "(e) the root rebuilt for a new def does not re-apply the first hydrate payload", "(e2) the root built for a new def takes no streamed rows, earlier or later" and "(j) a new def whose render is thrown away leaves the committed root alive".
- **The stream, from the first render.** A root built for the first `def` takes the streamed batches. `buildRoot` reads the batches already on the page through `readStreamed` and adds them to the root's `hydrate`, so they are buffered before any controller binds its key, the hydrating render reads them, and no fetch starts for them (`context.ts:228-247`). Before, the intake applied them in a passive effect after the commit: the client snippet in `../flows/ssr.md` rendered `loading…` where the server had rendered data, and each controller had already started the fetch. `streamCursors` keeps each such root's `StreamCursor`, the queue it read and how far. A reused uncommitted root catches up in render. A passive effect then calls `connectIntake(root, cursor)`, unless `streaming={false}` (`context.ts:602-609`), which applies only the batches past the cursor and each later one. So a re-run effect, after an `<Activity>` show or StrictMode's replay, never applies a batch twice. With `streaming={false}` the root gets a cursor at the start of the queue and nothing folded in. Pinned by `streaming-hydration.test.tsx`, "HydrationBoundary over a page streamed by …", and "(q) a retry first applies the batches that streamed in since its root was built".
- **Rendered on the server, the boundary leaks its root.** A server render runs no effects, so the cleanup that disposes the root never runs. In a development build the boundary warns once per process, through `warnServerBoundary`, when it renders with no `window` (`context.ts:168-177`, `context.ts:534`). The message names the fix: a per-request root through `OlasProvider`, disposed after the response. The once-gate is a module flag, because the mistake sits in the app's server entry and one message names it. Pinned by `coverage-server-env.test.tsx`, "warns once that its root is never disposed", and the jsdom lifecycle test asserts the browser path stays quiet.
...
`@kontsedal/olas-core/testing` exports `fakeField<T>(initial, overrides?)` and `fakeAsyncState<T>(overrides?)`. They produce shape-correct objects that satisfy `Field<T>` or `AsyncState<T>`, so a test can pass them straight into a component that calls `useField` or `useQuery` without building a real controller. See `packages/core/src/testing.ts:68-248`.

## Streaming SSR

The server half is a plugin. `createStreamingHydrator` returns `{ plugin, flush, dispose }`, and the plugin's `onWrite` captures the committed writes of the root it is installed in, meaning sources `'fetch'`, `'write'`, `'replace'` and `'commit'` (`streaming.ts:137-205`). It skips `'hydrate'`, `'optimistic'` and `'rollback'`. `createStreamingTransform(flush)` places the batches in React's HTML stream. On the client, `connectIntake` applies each batch through `root.hydrate(state)`, one signal `batch` per arriving batch, to every installed root (`streaming.ts:647-686`). `installStreamingIntake(root)` is that with a cursor at the start of the queue. The whole flow is `../flows/ssr.md`.

## Streaming SSR security (1.0)

`createStreamingTransform` writes a batch only where React's hydration never sees it: directly inside `<body>` for a whole document, or at the top level for a fragment, outside every `<Suspense>` boundary's comments, and right after a tag or a comment. `htmlBoundary()` in `streaming.ts` tracks the markup it passes through, byte by byte, with a stack of open elements and boundary regions. The old transform wrote a `<script>` after every chunk, and React's fixed-size chunks can end inside an attribute value, which was an XSS. The first fix wrote one at any chunk end in text, which put it inside a list item or a hidden segment and broke hydration (`pitfalls/stream-chunks-split-tags.md`).
