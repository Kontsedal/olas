...
The listing matches `packages/react/src/index.ts:1-47`. `useRoot` is at `context.ts:81-87` and `HydrationBoundary` at `context.ts:411-479`.
...
- **Unmount disposes, in its own passive effect.** A `useEffect` with `[]` deps disposes the owned root on unmount (`context.ts:460-466`). It must not be a layout effect: React runs layout-effect cleanups when a `<Suspense>` above hides content it already showed, and a hide is not an unmount. A layout-effect dispose killed the app root whenever a later suspension showed the fallback. The claim effect has no cleanup, because a cleanup there would also run when `root` changes. Pinned by "(l) a Suspense above that hides the committed boundary keeps its root".
