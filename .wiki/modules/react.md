...
The listing matches `packages/react/src/index.ts:1-47`. `useRoot` is at `context.ts:81-87` and `HydrationBoundary` at `context.ts:340-408`.
...
- **The sweep.** `armSweep` disposes a root that stays unclaimed for `ORPHAN_GRACE_MS` (ten seconds) after `root.waitForIdle()` resolves (`context.ts:246-266`). The countdown starts at idle because a child suspended on the root's own fetch is retried when that fetch settles, and the retry must still find the root. `ORPHAN_MAX_MS` (a minute) bounds the wait: a root with a hung fetch never goes idle, and it is disposed anyway. Each reuse restarts both timers. Pinned by "(m) an unclaimed root that never goes idle is disposed after a minute".
