...
```ts
import { isAbortError } from '@olas/core'
...
`isAbortError(err)` is exported from `@olas/core` — checks for `err instanceof DOMException && err.name === 'AbortError'`. Mode `serial` never aborts queued calls. Mode `parallel` never aborts.
...
### 8.7 Zod integration (`@olas/zod`)

Zod is the de facto schema library; we ship a small adapter rather than baking Zod into core (Zod is ~13 kb, opt-in). `@olas/zod` exports two helpers:

```ts
import { z } from 'zod'
import { zodValidator, formFromZod } from '@olas/zod'
...
Olas core stays Zod-free; `@olas/zod` has a peer dep on `zod ^3` and `@olas/core ^1`.
...
Adapters live in tiny separate packages (`@olas/react`, `@olas/vue`, `@olas/svelte`, vanilla). Roots are created **once** outside the UI (typically near `main.tsx`); the adapter resolves the root via context and exposes hooks for reading signals.
...
Selection state (which items are selected, the "anchor" for shift-click range select) recurs in every table / list with bulk actions. Use the `selection` composable from `@olas/core`:

```ts
import { selection } from '@olas/core'
...
| Package         | Purpose                                                                                                                             |
| --------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| `@olas/core`    | Everything: signals wrapper, controllers, ctx, lifecycle, caches, queries (incl. infinite), mutations, emitters, fields, forms, field arrays, validators, throttle/debounce, error handling, dehydrate/hydrate, devtools event bus. |
...
- `@olas/core` — peer: `@preact/signals-core ^1`.
- `@olas/react` — peer: `@olas/core ^1`, `react >= 18`.
- `@olas/persist` — peer: `@olas/core ^1`.
- `@olas/zod` — peer: `@olas/core ^1`, `zod ^3`.
...
The full public TypeScript surface. Internal types are not listed; anything in this section is exported from `@olas/core` (or `@olas/react` / `@olas/persist` where noted).
...
### 20.10 React adapter (`@olas/react`)
...
**`useField` fake helper.** For UI tests, `@olas/core/testing` exports `fakeField<T>(initial, overrides?)` and `fakeAsyncState<T>(overrides)` that produce shape-correct fakes:
...
### 20.11 Persistence adapter (`@olas/persist`)
...
// from @olas/core/testing  (sub-entry exception: testing is the one sub-path we ship)
...
The testing helper is the **only** sub-path export we ship (`@olas/core/testing`). It's separated because importing it in production builds is a smell — tree-shakers should drop it, but a sub-path makes the separation explicit.

---

## 21. Internal architecture

This section describes how `@olas/core` is organized internally. None of these types are exported — they're implementation detail. Listed here so contributors (and devtools) know the boundaries.
...
|---|---|---|
| `@olas/core` | **~10–14 kB** | Includes signals (preact-signals: ~1 kB), controllers, query client, mutations, forms (the biggest piece), scopes, errors, devtools event bus, SSR. |
| `@olas/react` | **~1–2 kB** | Thin wrapper over `useSyncExternalStore`. |
| `@olas/persist` | **~1 kB** | localStorage adapter + composable. |
| `@olas/zod` | **~2 kB + your Zod** | Adapter only; Zod itself is ~13 kB. |
