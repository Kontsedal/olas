...
- A second `suspend()` without `maxIdleTime` keeps an armed timer. A visibility hook that suspends a root already suspended with a bound therefore does not lift it. A second `suspend({ maxIdleTime })` restarts the timer with the new value.
- `suspend()` after `dispose()` does nothing and arms no timer.
...
**Projections.** `createQuery(ctx, query, { key, select })` reports `select(data)` instead of the cached value. The projection runs per subscriber, and the cache keeps the raw value. `select` never receives `undefined`: `data`, `firstValue()` and `refetch()` report `undefined` as it is.
...
type QuerySubscription<T> = {
  ...
  firstValue: () => Promise<T>        // the data now if there is some, else the first success — for SSR / navigation guards
}
```
...
**`firstValue()` resolves at once when the entry holds data** (`!== undefined`, §6.4), even while a background refetch runs or after one failed. With no data it waits for the first success and rejects on the first failure. A disabled subscription's `firstValue()` waits until it is enabled and loaded (§5.2). It never waits on nothing. On an idle entry with no data and no fetch in flight or parked, it starts a fetch. A cancelled first load and a `reset()` of a failed one leave an entry that way. A `cancel()` while it waits makes the entry fetch again one microtask later, unless data arrived meanwhile: the `setData` that follows a cancel in the optimistic recipe resolves it with the guess. So a Suspense boundary recovers by loading, and a cancel does not reach an error boundary. The data must belong to the current key. After a key change, a `createCache` local cache with `keepPreviousData` keeps showing the previous key's data, and its `firstValue()` settles with the fetch for the new key instead, as a shared query's subscription does.
...
**`isStale` follows the server, and `lastUpdatedAt` follows the data.** `lastUpdatedAt` is when `data` last changed through a fetch, a hydrated row or a write, an optimistic `setData` included. `isStale` is measured from the last write of server truth: a fetch (for an infinite query, a page too), a hydrated row, or a canonical `write` or `replace`. An optimistic `setData` is a guess, so it moves `lastUpdatedAt` and leaves `isStale` and its timer alone (§5.9).
...
**A key is identified by the value JSON round-trips it to.** `dehydrate` ships raw key args, and the payload reaches the client through JSON (§15), as a persisted cache does (§13.4). A key that JSON rewrites would otherwise hash differently after the trip, and the client could never adopt its entry. So an object member holding `undefined` counts as absent, and `undefined` in an array counts as `null`. A Date counts as its ISO string, and an invalid Date as `null`. `NaN`, `Infinity` and `-Infinity` count as `null`, and `-0` counts as `0`. `{ q: undefined }` and `{}` therefore name one entry. Every other value keeps its type: `1` and `'1'` stay apart, as do `null` and `'null'`. A bigint keeps its own identity, and JSON has no form for one, so an entry keyed by a bigint cannot travel through JSON. Functions, symbols, `Map`, `Set`, class instances and cycles throw.
...
- The key of a `createCache` local cache changes. For a shared query, a key change releases the old entry instead, and its fetch keeps running for the entry's other subscribers and its gc window. A local cache compares keys by the hash of §5.4, as a shared query does. A key thunk that re-runs to an equal key, such as the same id read from a new object, changes nothing: the cache neither refetches nor aborts. A key the hash cannot encode, such as a class instance, compares element by element.
...
- `online` (default) — a fetch requested while offline is **deferred**, not run; it resumes automatically on the next reconnect. The entry reports `isPaused: true` while deferred. The request supersedes a fetch already in flight, as a request made online does, so that older response never lands. On a local cache whose key changed offline, it would carry the old key's data.
...
- `staleTime` — on subscribe and access, refetch if the server data is older than this.
...
**Staleness runs on the server's clock.** The age that `staleTime` limits starts at the entry's last fetch, hydrated row, or canonical `write` or `replace`. On an infinite query a page fetch counts too. The `isStale` signal and the refetch decision read that same clock. An optimistic `setData` does not reset it. A guess made at the edge of the window therefore does not keep old server data fresh, during the write, after a rollback, or after a finalize.

**A live optimistic write holds back the staleness-driven fetch.** A subscriber that acquires the entry, a `resume()`, a focus or reconnect refetch and a `prefetch` each consult staleness. While an optimistic write on the entry is live, they start no fetch, because its response would land over the guess on screen (§6.4). A `prefetch` then resolves with the data on hand. When the last live write settles, by rollback or finalize, the entry runs one fetch for all of them, if something still holds the entry, no fetch is in flight, and the entry is still stale. That decision waits one microtask, so an `invalidate()` in the mutation's `onSuccess` or `onSettled` starts the only request. An invalidation still standing wins over a live write, and a subscriber that arrives then fetches at once (§5.7). Settling is not a trigger of its own: with no fetch held back, a settle fetches nothing. A `refetchInterval` tick does not consult staleness, so a live write does not hold it back.

A focus, reconnect or interval refetch joins a fetch already in flight rather than restarting it. It starts no fetch of its own for an entry whose fetch is parked for the network (§5.5). While `navigator.onLine` reads false it does nothing, so an interval ticking offline parks nothing new. Once the network is back it runs the parked fetch, which settles every waiter. The entry's reconnect drain does the same on an `online` event, and one `online` event makes one request. A park still ends when that event never reaches the entry: a worker has no `window` to fire it, and a browser can fire it while `navigator.onLine` still reads false. There the next focus, reconnect or interval trigger runs it. Any fetch made once the network is back ends the park too, a `refetch()` included. Its outcome settles the parked requests, and the entry stops waiting for the `online` event, so a later event makes no request and aborts nothing.
...
**Refetch semantics.** A refetch of an infinite entry **re-fetches every currently-loaded page** in order, not only the first. It starts at the first loaded page's param and chains each next param via `getNextPageParam` off the freshly-fetched pages, as TanStack does. A list paged backwards to `[-2, -1, 0]` therefore refetches as `[-2, -1, 0]`. A first load, with no page loaded, starts at `initialPageParam`. The triggers are `refetchInterval`, `invalidate()`, `refetch()`, focus and reconnect. Pages update atomically at the end, so a scrolled-down list never flashes down to one page mid-refetch. This matches TanStack. A refetch that finds the dataset has shrunk, because a page now returns `getNextPageParam === null` earlier, keeps only the pages that still exist.

The `flat` selector is configurable per-query via an `itemsOf: (page) => page.items` field. If omitted, `flat` equals `pages`.

Invalidation is a refetch like the others: it marks the entry stale and re-fetches every loaded page from the first one's param, and the pages swap at the end. There is no "partial invalidate". For "keep only the most recent N pages", `write` the pages you want to keep.

Pages written without their params keep the params aligned. A write that adds pages pads with the last param, or with `initialPageParam` when the entry held none. `fetchNextPage()` and `fetchPreviousPage()` called before any page has loaded join the first load in flight, and start one only when none is running.
...
- **`serial`** — calls are queued and executed one at a time in order. Use when ordering matters and you can't drop intermediates. Plugins hear a call that waits as `'queued'` at once (§13.1).
...
- Rolling back a **non-top** snapshot splices that layer's baseline out of the chain and threads it onto the next layer ("chain-splice"). The layers above it still hold its delta, so the entry replays them. Each one's updater runs again over the baseline below it, and the data on screen becomes the top layer's result. The failed layer's delta leaves the screen at once, and every still-pending delta stays.
...
**A live snapshot holds back staleness refetches.** Those are the fetches a subscriber, a `resume()`, a focus or reconnect refetch and a `prefetch` start on a stale entry. While a snapshot is live, none of them fetches, and the entry runs one fetch when the last snapshot settles, if something still holds it (§5.9). A snapshot does not reset staleness either. After a rollback or a finalize the entry is as stale as the server data it last received, so a guess never passes for fresh server data.

**The replay follows the same rules as a commit.** A layer that a fetch or a hydrated row replaced while it was live is not replayed: the read may already hold its change, and it passes the read's value through. Two cases cannot be replayed exactly. A plain value, an updater that takes no `prev` such as `() => value`, cannot drop a change it captured from the screen. An updater can also throw on the new baseline, and a throw leaves the screen as it was. In both cases the entry marks itself stale and refetches once the last live layer settles, if something still holds it. With `structuralShare` on, the default, a replay that changes nothing keeps the data's reference and reports no write. Without the replay, a commit of a layer above the failed one reported the failed guess as committed truth.

**A committed layer folds into the baselines below it.** `finalize()` commits a layer as server truth, as a successful mutation does. The layers still live below it captured their baselines before it applied, so the entry re-runs the committed layer's updater on each of them. Two parallel toggles show why: A applies, B applies, B succeeds and A fails. A's rollback then restores its baseline with B's change in it, not the value from before both. The fold is skipped when a fetch or a hydrated row landed while the committed layer was live. That read replaced the baselines with server truth, which may already hold the change, and re-running a toggle there would undo it. For conflicting updates, meaning two mutations writing the same field, prefer `concurrency: 'serial'` or explicit conflict resolution in `onMutate`.
...
All three rebase live optimistic snapshots, so a rollback restores what was written rather than a baseline captured before it. `write` is a patch, so the entry re-runs its updater on each live baseline. Setting each baseline to the data on screen would keep the guesses in it after a rollback. `replace` is the whole record, so its value becomes each baseline.

**When a baseline cannot be re-derived, the entry reconciles.** An updater can throw on a baseline it was not written for. The `undefined` under a guess made before the first load is one such baseline. That baseline keeps its value, and the entry marks itself stale, as an invalidation does. It fetches once the last live layer settles, if something still holds it, so a rollback does not leave a value known to be wrong. A development build warns.
...
draft.reset()                 // restore initial value, clear dirty/touched + server errors, re-check it
draft.markTouched()
...
A member typed `ReadSignal` is read-only at run time too. `field.touched`, `field.isDirty`, `field.isValidating`, `form.isSubmitting`, `form.submitCount`, `form.submitError` and `array.items` have no `set`, and assigning their `value` throws. A cast therefore cannot write `array.items` past the disposal of the rows it drops. Each member keeps one identity, so a hook that subscribes to it keeps one subscription.

**An object value can be edited in place and set back.** `set` of the object the field already holds counts as a change, as a Svelte store's `set` does. Subscribers hear it, the validators run, and `isDirty` is recomputed. That is how Svelte writes a nested bind, `bind:value={$person.first}` (§16.3). A primitive equal to the current value (`Object.is`) is still no change. The field keeps what `reset()` restores as a copy of its own. Plain objects and arrays are copied at every depth, and a class instance, a `Date` or a `Map` is kept by reference. So an edit in place cannot reach the baseline, and the copy compares equal, so it does not read as dirty. A `Form` keeps a fixed `initial` the same way, and a `FieldArray` its initial rows. The value itself stays the object the caller passed, so `field.set(x)` leaves `field.value === x`.
...
**Sync validators run first; async validators start only after every sync one passes.** A pass calls the sync validators in declared order and collects all their errors. If any failed, the pass settles on those errors and does not call the async validators at all, so `[required(), checkUsername]` never sends `checkUsername('')`. A pass cannot tell a sync validator from an async one before calling it. A validator therefore counts as async when it is declared `async`, or once it has returned a promise. One that returns a promise without being declared `async` is called with the sync ones until it has, and a sync failure in that pass aborts it. The same order holds for form-level and array-level validators (§8.3, §8.5).

**A reset leaves the field as a fresh field with its initial value would be.** A value that changes re-runs every validator. A value that already matches the initial one by content stays as it is and re-runs the sync validators, so a pristine `required()` field still reads invalid after `reset()`. The async ones are not called again for an unchanged value: a reset should not re-send a request for a value the user did not change. `revalidate()`, `validate()` and `submit()` re-run them. A `'blur'` or `'submit'` field re-locks, so it shows no errors.

**A check in flight ends with its field.** `reset()` drops it: its result does not land, even from a validator that ignores its `AbortSignal`. `dispose()` ends it too. `isValidating` drops to `false`, and a `revalidate()`, `validate()` or `submit()` waiting on it resolves. A row removed from a field array mid-submit therefore does not hold the submission.
...
A `fn` that throws synchronously, or returns no promise, is a bug in the validator rather than a failed check. It fails the pass the way a throwing sync validator does: the message shows on the field and the error reaches the root's `onError` as `kind: 'effect'`. The pass settles, so `isValidating` drops and a `submit()` waiting on it resolves.
...
form.dirtyFields  // ReadSignal<string[]> — dotted paths of the dirty leaves, for a PATCH payload (§8.5)
...
**Server errors on a nested form or field array.** A `setErrors` path can name a nested form or a field array, or `''` for the form itself. The messages then land on that node's `topLevelErrors`, where `flatErrors` and `isValid` see them. They stay while the node's value stays. The next change anywhere in it clears them, the way a field's next `set()` clears its own. `reset()`, `setAsInitial()` and an empty list clear them too.
...
Each issue's `path` walks the schema exactly like `flatErrors` paths (object keys, numeric array indices); an **empty** path lands in `topLevelErrors`. A whole-form Standard-Schema validator (`validator(schema)` and `zodValidator(objectSchema)`) works the same way — its issues keep their `path`, so `z.object({...}).refine(fn, { path: ['confirm'] })` lands on `confirm`. Field-targeted messages are a **third error channel**, beside a field's own validator output and `setErrors` server errors. They merge into the field's visible `errors`, and are **cleared and recomputed on every form-level run**. Fixing the mismatch therefore removes them, while a field's own `set()` does not. Each routing form keeps its own list on the target. When an inner form's rule and an outer form's rule both target one field, fixing one rule removes only its message, and the other rule's message stays. The form-level run is their only writer: a field's `reset()` and `setAsInitial()` leave them, and so does a form's `reset()` for its `topLevelErrors`. A reset that changes the form's value re-runs the validators, and one that does not leaves their last result standing, so a rule that still fails stays on screen. An unresolvable path falls back to `topLevelErrors` rather than vanishing. The same mechanism applies to array-level validators on a `FieldArray`, whose paths are `[index, ...]`.
...
The factory passed to `createFieldArray` runs once per `add()` and `insert()` to construct a fresh sub-form (or sub-field), and receives the value passed to them. Each item is owned by the array; removing it disposes the underlying form. A disposed item drops its registration on the controller (§3.4), so `remove`, `clear`, `set` and `reset` leave nothing behind however often they run.

**Structural dirtiness.** `add`, `insert`, `remove`, `move`, and `clear` mark the array **dirty** — `isDirty` is `true` after any of them, not only after a per-item edit. This is what makes the reactive-`initial` guard (§8.4) safe. Once the user adds or removes a row, the default `resetOnInitialChange: 'when-clean'` stops re-seating. A background refetch of `initial: () => queryData` therefore cannot silently delete the rows the user just added. `reset()` — and an `initial`-driven re-seat — clears the structural dirt back to the clean baseline. `form.dirtyFields` lists a structurally dirty array by its own path, such as `tags`, and none of its items. After a row moves or goes, the indices no longer match the baseline, so a PATCH sends the whole array. A structurally clean array lists its dirty items, such as `tags[1]`.

**A `null` row builds from the item's defaults.** An item factory of the documented shape passes a row's `null` on as `createForm(ctx, schema, { initial: null })`. A form takes an `initial` of `null` as none. A throw while a reactive `initial()` seats the form, from an item factory for one, reaches `onError` rather than the write that re-ran the thunk (§8.4). The array builds the new rows before it drops the old ones, so a throw leaves it as it was.
...
Semantics:
- It validates first, unless `validateBeforeSubmit: false`. An invalid form marks every leaf touched, skips the handler and resolves `{ ok: false, reason: 'invalid' }`. A form disposed during that validation skips the handler and resolves `{ ok: false, reason: 'disposed' }`.
- It validates first from inside `batch()` or an effect too. A write in the same batch starts its async check when the batch ends, and the handler waits for that check. `revalidate()` and `validate()` wait the same way.
...
Each leaf field's initial is the Zod schema's default if present, otherwise an empty value for that type (`''` for string, `0` for number, etc.). A default on an object or an array seeds that nested form or field array as a whole, as `schema.parse({})` fills the key. The `initial` option overrides it, as a partial value or as a tracked function that re-seats a clean form (§8.4), with the same `resetOnInitialChange` choices. `extraValidators` adds validators to named fields beside the schema's own.
...
Both forms take an optional `deps` override for the children, like `ctx.child`. A collection also exposes `suspendItem(key)`, `resumeItem(key)` and `isItemSuspended(key)`, so a virtualized list can freeze the rows scrolled out of view without disposing them (§4.1). A construction error in one item goes to `onError` with `kind: 'construction'`, and the collection skips that item (§12.1). That covers a throw from `keyOf`, `propsOf`, `factory` or the child controller's own factory, and the rest of the diff still lands. A kept key whose `factory` throws keeps its child, because the factory named no controller to rebuild it as. A key rebuilt for a new controller keeps a `suspendItem(key)`: the new child is built suspended. Once the owner disposes, `items` is empty and `has(key)` is `false`.
...
6. **`root.onError` only fires for construction errors that happen *after* the root is alive.** Two examples: an `effect` that constructs a `ctx.child` lazily, and a `ctx.collection` factory that throws when a new item arrives. Those throws go to `onError` with `kind: 'construction'` and do not propagate. The collection skips the bad item, so the UI sees one fewer entry. The kind follows the throw rather than the callback that caught it. A `ctx.child` or `ctx.attach` whose factory throws reports `'construction'` from inside an effect, a `ctx.on` handler or a mutation hook too. A factory that throws a primitive, such as a string, cannot be told apart and keeps the caller's kind.
...
| `setData(id, key, updater, options?)` | An optimistic write, as `setData` in `onMutate` (§6.4). It pushes a layer, reports `'optimistic'`, leaves the stale clock and a fetch in flight alone, and returns the `Snapshot` the plugin settles with `rollback()` or `finalize()`. The settle reports as an app's does. `undefined` when the root holds no entry for the key. |
...
`write`, `replace` and `setData` take `{ pageParams }` for an infinite query, so the entry's cursors stay aligned with the new pages.
...
A `WriteEvent` carries `query`, `key`, `data`, `updatedAt`, `source`, `origin` and `server`. `query` is the `QueryRef`, and `data` is the value after the write, or the pages for an infinite query. An infinite query's write also carries `pageParams`, one per page. `server` is the entry's server truth, as `dehydrate()` ships it (§15). It holds the data beneath every live optimistic layer, with canonical writes and commits folded in, its `updatedAt` when the server last said it, and `pageParams` for an infinite query. With no layer live it holds the same value as `data`. It is absent when the entry holds neither data nor a server answer. Each write reports exactly once. A hydration, for example, reports one `'hydrate'` write and no fetch.
...
| `'commit'` | An optimistic layer was committed as server truth: its snapshot was finalized, as a successful mutation's is. |
...
While a layer is live, `data` holds the guess whatever the source is: a `'write'` made under a guess carries it too. A plugin that persists server truth therefore stores `server`, and a plugin that relays a guess writes it on the other side with `setData`, so it stays a guess there. `'optimistic'` and `'rollback'` are guesses the server has not confirmed.

**A commit is reported once no optimistic layer is live.** Its `data` is then the data on screen, which holds no pending guess. A layer that commits while another is still live reports nothing then. The settle that clears the last live layer reports the commit, even when that settle is a rollback, and a `'commit'` then takes the place of the `'rollback'`. A fetch or a hydrated row that lands in between carries the server truth itself, and the commit reports nothing more. A commit's `updatedAt` is the time the server last answered, by a fetch, a hydrated row or a canonical write, because a commit does not restart the stale clock (§5.9). It is `0` when the server never answered.

#### Mutation events

`onMutation` fires for every run, with or without an `id`. Each run reports `'start'` after `onMutate` and before the first `mutate` call. Then it reports exactly one outcome: `'success'`, `'error'` once retries are exhausted, or `'cancel'` for a supersede, a `reset()` or the owner's disposal. A `MutationEvent` carries `mutation`, its `MutationRef`, and `runId`, `variables` on every phase, `result` on success, `error` on error, `reason` on cancel, and `origin`. `origin` names the plugin that started the run through `host.mutations.run`, and is `undefined` otherwise.

`reason` says why a run was cancelled. `'superseded'` means a newer `latest-wins` run replaced it (§6.1), and `'reset'` means `reset()` dropped it. Both are the app discarding the run on purpose. `'dispose'` means the controller that owned the run was disposed (§6.2). The screen is gone, and the write it asked for may still be wanted. A root's own disposal reports nothing, because plugin delivery closes first (see Lifecycle below).

A `serial` run that has to wait behind another reports `'queued'` when `run(...)` is called (§6.1). It carries the `runId` the run keeps, and the run reports `'start'` when its turn comes. A plugin can therefore record a waiting run before the run ahead of it settles. A queued run that never starts still reports exactly one outcome: `'cancel'` when `reset()` or the owner's disposal drops it, or `'error'` when its `onMutate` throws. A run that did not wait reports nothing when its `onMutate` throws or is cancelled, since it reported nothing before.
...
- **A root factory throw tears down in the dispose order.** Delivery closes before the factory's partial state rolls back, so no plugin hears the rollback, such as an entry deactivating. Then the plugins dispose in reverse order, and the query client last.
...
- `'optimistic'` and `'rollback'` writes by default, so a peer shows a pending edit before the server confirms it. `optimistic: false` limits the channel to canonical writes, commits and invalidations, and a canonical write made under a guess then sends the data beneath it.
- `'commit'` writes, so a peer ends on the committed value whether or not it showed the guess.
- The write's `source`, and, for a `'write'`, `'replace'` or `'rollback'` made under a live guess, the server truth beneath it.
- Never `'fetch'` or `'hydrate'` writes. Every tab runs its own fetcher, and relaying results would be quadratic noise.

**Receiving.** A peer's write goes through `host.queries`, so it carries the plugin's name as `origin` and is never mirrored back. A tab applies it only to an entry it holds, and a key it has never loaded fetches for itself on first subscribe. Each source is applied as what it is. A `'write'` is a `write` and a `'replace'` a `replace`, which supersedes a fetch the tab has in flight (§6.4), each of the peer's server truth. A peer's guess is shown through `setData`, so it restarts no stale clock, `hasPendingMutations` reads `true`, and a persister does not store it (§13.4). The peer's next message about the entry replaces it. A `'rollback'` removes it, and a `'commit'` makes the data the tab's own as a commit does, leaving the stale clock where the server set it. A guess its peer says nothing more about for 30 seconds is rolled back, since a tab that closes mid-mutation never settles it. A message with no `source`, as versions before 1.0 send, is a `'write'`, and one with a source the tab does not know is dropped. A peer's invalidation refetches an entry only when that tab subscribes to it. `validate(queryId, data)` lets a tab reject a payload shape it did not expect; any same-origin script can post on the channel (§22).

**Echo prevention is layered.** First, the plugin mirrors no write stamped with its own name. Second, messages carry a `sourceId`, and a root drops its own. Third, messages carry a monotonic `msgId`, and a receiver drops a duplicate or out-of-order message, and one whose `msgId` is not a safe non-negative integer. A receiver moves a peer's `msgId` cursor only for a message it applied. A `msgId` 64 or more below the cursor means a forged message pushed the cursor ahead: the receiver applies that message and restarts the cursor from it, so one planted message cannot silence a peer.

**Servers.** Without a `channelFactory`, the plugin opens a channel only in a browser tab (a scope with a `document`) or a web worker. Node, Bun and Deno define `BroadcastChannel` too, but there it reaches every root in the process and across worker threads or isolates, so per-request server roots would read each other's writes. On those runtimes, and where `BroadcastChannel` is not defined, the plugin installs no hooks. A `channelFactory` opens a channel wherever it returns one.
...
**A `dedupeBy` entry holds the newest write.** A run that collapses onto an entry writes no entry of its own, and settles the one it collapsed onto. Once the run that wrote the entry has settled and the entry stays, the entry holds the variables of the newest run collapsed onto it: a `latest-wins` supersede leaves the successor's variables on disk, not the superseded run's. A run that collapses onto an entry kept for a replay, after a dispose or a failure worth a retry, rewrites it with its own variables before its first attempt. A queued `serial` run never collapses and never takes a key, since the run ahead of it settles before it starts. A success deletes the entry only when no newer run's variables ride on it. A replay of the entry, or the run that wrote it, may succeed while a newer run collapsed onto it is still executing. The entry then stays and holds that newer run's variables. A success under a key also deletes the entries this session kept under the same key for a replay.

**Replay.** At setup, and again on reconnect, the plugin lists the keys under `keyPrefix`, checks each entry, and groups the entries by mutation id in `seq` order. Two tabs can mint the same `seq`, so `runId` breaks a tie, and every tab sorts the same entries the same way. It replays each group serially through `host.mutations.run`, so the definition's `retry` applies and `mutate` receives the root's `deps` (§13.1). An entry that stays in storage ends its group's pass: a failure worth a retry, or a run executing the entry. The group's later entries wait for the next pass. So a later entry never reaches the server ahead of an earlier one. It replays only a definition whose `meta.persist` is `true`, and it requires each entry's storage key to match its contents (§22). A startup replay that begins online is `track`ed, so `root.waitForIdle()` waits for it; one that begins offline waits for the reconnect instead. A cross-tab lock, through the Web Locks API with a `localStorage` lease as the fallback, keeps two tabs from replaying the same entries at once. A pass skips an entry a live run rides on, in its own tab or in another. Each live run marks its entry as its tab's until it settles: a Web Lock named for the entry, or a `localStorage` lease a heartbeat refreshes. A closed tab's locks are released and an unrefreshed lease expires after 30 seconds, so a dead tab's entries replay on a later pass.
...
- The plugin stores each opted-in entry's server truth, the write event's `server` (§13.1): what `dehydrate()` would ship, the data beneath any live optimistic write. So a guess is never stored, whatever write carried it, and a `'commit'` is. A commit on an entry the server never answered for is stamped `0`, and the plugin skips it, since `maxAgeMs` has nothing to age it by. It throttles the storage writes, one per `throttleMs` (default 1000). An entry the cache collects is dropped from storage too.
- Each write carries the whole cache, and every tab of the app writes the same key. So each write reads what storage holds first, with or without `restore`, and merges this session's writes into it. An entry this session never binds stays in storage until it passes `maxAgeMs`, and so does an entry another tab wrote. When storage and the session both hold a copy of one entry, the newer `lastUpdatedAt` wins. An entry the session garbage-collected stays out, unless storage holds a newer copy. A read that fails holds the write, and the next write reads storage again first, so a transient read error cannot delete the stored entries. A stored entry of a query the root has used and `include` rejects is dropped. The read and the write are not one transaction: two tabs writing at once can each miss the other's newest entry, and the next write of the tab that lost it restores it.
...
A new subscriber first receives a replay of the live controller tree, so a panel that mounts after `createRoot` sees the existing tree at once. No other event type is buffered. The replay follows the order construction finished in, because `controller:constructed` is sent after a factory returns. A child built inside its parent's factory therefore arrives before the parent, and one attached later arrives after it.

A subscriber runs untracked. Many events are sent from inside an effect, such as `field:validated` from a field's validator pass. A subscriber that reads a signal therefore adds no dependency to that effect, and a write to the signal does not re-run the pass.
...
`dehydrate()` only serializes the **query client cache**: `id`, `key`, `data` and `lastUpdatedAt` for every entry that holds server truth. `status` is not the test. It reads `'pending'` over the data during a background refetch, and `'error'` over it after a failed one (§5.3), so an entry at either moment still ships its data. An entry with no data and no server answer, such as one whose first fetch is running or failed, is left out.

**A row carries server truth, stamped with the time the server said it.** The stamp is the entry's last fetch, hydrated row or canonical write. It is not `lastUpdatedAt`, which an optimistic `setData` moves and its rollback leaves. Under a live optimistic write the row carries the data beneath the guess, with every canonical write and commit folded in, so a guess never ships as server truth. A committed guess ships, stamped with the last server answer, since a commit does not make data fresh (§5.9). It is stamped `0` when the server never answered. Controller state isn't serialized — controllers reconstruct from their props on the client. **Infinite queries dehydrate too.** Their entries carry the pages as `data` and the page params as `pageParams`, so the client continues paging where the server stopped (§5.11).

Each serialized entry carries its query's `id` alongside its key. Every shared query has an `id` (§5.2), so every entry that holds data dehydrates. IDs must be unique per query and identical in server and client bundles; registration order never identifies hydrated data. Hydration is namespaced by `id + key hash`, so two queries whose keys match cannot adopt one another's data. The key hash is taken over the value JSON round-trips the key to (§5.4), so a key holding an `undefined` member, a Date or `NaN` is adopted after the JSON trip. A bigint key has no JSON form, and `serializeForScript` throws on it. An entry that arrives before any subscription binds its key waits in a buffer, and the first binding adopts it. The buffer keeps one row per key: a row stamped before the one already waiting is dropped. A row whose `data` is `undefined` gives its entry `status: 'success'` on both paths, the buffered one and a bound entry. Hand-authored payloads must use the target query's `id`. Core checks the payload's `version` and each entry's shape, and skips an entry it cannot read (§22).
...
- **`useQuery(subscription)`** returns the whole `AsyncState` as plain values, with the actions `refetch`, `reset` and `cancel`. The values are `data`, `error`, `status`, `isLoading`, `isFetching`, `isStale`, `isPaused`, `isEnabled`, `lastUpdatedAt` and `hasPendingMutations`. **It re-renders only for the fields the component read during render.** The result is a tracked snapshot. Its getters record each field read, and the subscription notifies React only when one of those moves. A read after commit returns the live value, even after React has thrown away a later render of the component. Until the component reads anything, every change notifies.
- **`useQuery(subscription, { suspense: true })`**, or `useSuspenseQuery(subscription)`, suspends until the first load settles and types `data` as present. It waits for a settled load, not for `data` to be defined: a load can succeed with `undefined`, for a `select` over an optional field or a fetcher that resolves nothing, and the hook returns that. A refetch after a settled load never suspends. On a disabled query it suspends until the query is enabled and loaded (§5.2), with a one-time development warning.
...
- On one controller, the two share their bookkeeping, and the controller resumes only when neither has a reason left to keep it suspended. A wrapper that first mounts on a hidden tab waits for the tab to show. A tab that shows does not resume a controller whose last wrapper has unmounted.

Without these, you call `suspend()` and `resume()` yourself; the adapter doesn't drive lifecycle implicitly.

**SSR and streaming.** `<HydrationBoundary def={appController} options={rootOptions}>` builds the client root from `options`, including `options.hydrate`, and provides it. By default it takes the streamed batches, and it keeps one root through a StrictMode remount. It builds the root during render, and React can discard a render before it commits, for instance when a child suspends. A retry reuses the root that render built: the same element's, or an unclaimed root built from the same `def`, the same `hydrate` object and `deps` with the same members, which covers a parent that re-creates the element on every retry. When those options change between attempts, the retry builds a new root, and a development build warns once. A root that no commit claims is disposed about ten seconds after its work goes idle, or after a minute if it never goes idle. A `<Suspense>` above the boundary that later hides its content does not dispose the root, because a hide is not an unmount. An `<Activity>` above that hides the boundary does not dispose it either, but React runs the same effect cleanups for that hide as for an unmount, and gives no way to tell the two apart. So the cleanup suspends the root, and disposes it a minute later unless the boundary's effects run again first. A hide shorter than a minute keeps the root and its state. After a longer one, the boundary builds a fresh root from `options` when it shows. An unmount therefore disposes the root a minute late, and the root stays suspended until then. A render for a new `def` leaves the committed root alone, and the commit that installs the new root disposes the old one. A server render runs no effects, so a boundary rendered there builds a root per render that nothing disposes; a development build warns once, naming `OlasProvider` with a per-request root as the fix. On the server, `createStreamingHydrator({ nonce? })` returns a `plugin` for the server root and a `flush()` that drains the settled entries as one `<script>` tag. `createStreamingTransform(flush)` places those tags into a `renderToReadableStream` stream only where React's hydration never sees them. React writes fixed-size chunks, so a chunk can end inside a tag, an attribute or a text node. React hydrates every element it rendered, and the content of every `<Suspense>` boundary, and its reveal moves a completed segment, `<div hidden id="S:0">`, into its boundary. It skips a stray element only in the root container and directly inside `<html>` and `<body>`. So a batch goes directly inside `<body>` for a whole document, or at the top level for a fragment, outside every boundary's comments and right after a tag or a comment. It never goes into `<head>`, a text node, a tag, a comment, raw text or a hydrated element. The transform writes a pending batch at the first such point in a chunk, so data goes out before the markup that reads it, and holds it through a chunk with no such point. It drains once more when the stream closes, unless the stream ends inside markup. The transform wraps React's own stream; an HTML template of the app's goes around its output. `OLAS_BOOTSTRAP_SCRIPT` goes into React's `bootstrapScriptContent`. The batches already on the page when the boundary builds its root go into that root's `hydrate`, so the hydrating render reads them and no fetch starts for them. A retry that reuses an unclaimed root first applies the batches that arrived since it was built. Later batches arrive through the intake. A root built for a new `def` takes no streamed batches, since they describe the first root's tree. `installStreamingIntake(root)` connects a client root that is not built by a `HydrationBoundary`. The payload goes through `serializeForScript`, and the `nonce` covers a Content-Security-Policy (§22).
...
`olasPlugin(root)` provides the root to the whole app. `useRoot()` returns its `api` and is typed by the same `Register` augmentation, declared on `@kontsedal/olas-vue`. `useValue(signal)` returns a read-only `Ref<T>`, and `useQuery`, `useInfiniteQuery`, `useField` and `useMutation` return one ref per field (`Refs<T>`) plus the actions. Vue's own dependency tracking re-renders a template only for the refs it read, so the adapter tracks nothing itself. A ref's getter reads `signal.peek()`, so a write is visible to the next read before Vue flushes, inside a `batch` too. `useValue(signal, { isEqual })` keeps returning the value it last returned while `isEqual` calls the new one equal to it, as React's `useValue` does, so a component that re-renders for another reason shows the value it showed before. `useField(field).value` is a writable ref that reads the same way and writes through `field.set`, which makes it work with `v-model="value"`. `v-model` on a member of an object-valued field, `v-model="value.name"`, assigns the member on the field's own value object: nothing calls `field.set`, so the field is not notified and no validator runs. The field's baseline is a copy of its own, so `reset()` still restores the value from before the edit. Bind a writable `computed` that calls `set` with a new object, or a `Form`'s leaf field. `useRoot()` outside a component's `setup()` throws an olas error that names `setup()` and `app.runWithContext()`. `useMutation` returns `mutate` and `run` with the React adapter's semantics. Every subscription ends with the current effect scope. A hook called outside one still returns working refs, but nothing ends their subscriptions, so a development build warns once per hook, naming it. Vue never stops a component's scope on the server, so during a server render the refs read their signals and subscribe to nothing.
...
An Olas `ReadSignal` already satisfies Svelte's store contract: `subscribe(run)` calls `run` with the current value at once and on every change, and returns the unsubscribe. So `$signal` works on a `signal`, a `computed`, a `Field`, a `Form` or a `FieldArray` with no wrapper. A `Field` also has `set`, which makes it a writable store for `bind:value`. A `fieldStore` binds by its member, `bind:value={$state.value}`: each subscriber gets its own copy of the state, and `set` writes a copy's `value` to the field. Svelte treats a store's object value as changed on every `set`, and writes a nested bind, `bind:value={$person.value.first}`, by assigning the member in place and passing the object back to `set`. So the copy holds a shallow copy of a plain-object or array value, the assignment lands there, and `set` writes a new value: validators run, `isDirty` follows, and the field's baseline stays as it was. A member two levels down lands on the object the field holds, and the field still hears it and keeps its baseline. A raw `Field` bound the same way, `bind:value={$person.first}`, works too (§8.1). Svelte assigns the member on the field's own value object and passes that object back to `set`, and a `set` of the object the field holds counts as a change. So validators run and `isDirty` follows, at any depth. The field keeps its baseline as a copy of its own, so `reset()` restores the value from before the edit. The edit does change the object in place, and that object is the one last passed to the field. A field seated from query data holds the query's object, so the edit reaches the cache. Bind through `fieldStore`, whose copy stops one level down, or a `Form`'s leaf fields when the value is shared. A `Form` or a `FieldArray` also has `set`, but binding one of its members, `bind:value={$form.name}`, makes Svelte assign that member on the form's current value object in place before it calls `set`. The form ends right, and a value object the app held earlier changes under it. Bind the leaf field instead: `bind:value={$name}` on `form.fields.name`. `setRoot(root)` provides the root to the components below, and `getRoot()` returns its `api`; both are called during component initialization, and `getRoot()` is typed by `Register` on `@kontsedal/olas-svelte`. `queryStore`, `infiniteQueryStore`, `fieldStore` and `mutationStore` turn a multi-signal object into one store of its state, with the actions beside it. `mutationStore` carries `mutate`, `run` and `reset`.
...
The parity suite runs seven scenarios through React, Preact, Vue and Svelte. They cover a signal, the unsubscribe on unmount, a query, an infinite query, a field with validation, a fire-and-forget mutation, and `useValue`'s `isEqual` keeping the value shown. Svelte skips the last one: a signal is its store as it is, with no `isEqual` to pass. A pass shows that each adapter reads the same signals and re-renders on the same writes. It also shows that each passes the same actions through and lets go of its subscriptions on unmount. It does not measure speed, and it does not show that each view is idiomatic for its framework.
...
- On import and construction failure, `status` flips to `'error'` and `error.value` carries the cause; `root.onError` fires with `kind: 'construction'`. A loader that throws synchronously, or returns no promise, fails the same way, and `load()` returns a rejected promise rather than throwing.
- Parent disposal disposes the lazy child (if loaded) and aborts an in-flight load.
- A disposed handle, by its `dispose()` or by the parent's, reads `status: 'idle'` with `api` and `error` undefined, and its `load()` rejects. A load in flight then rejects too.
...
- shift-click → range from anchor (last clicked) to `id`, using `ordered` to define the range. `ordered` may be the row ids in order, or a `Map` from id to index, which a large list can build once. With a `Map`, the range is every id whose index lies between the two ends, in whatever order the `Map` was filled.

The anchor is the row last clicked. A meta-click that deselects the anchor keeps it as the anchor, as a file manager does, and so does `toggle(id)`. `deselect(id)` of the anchor clears it, so the next shift-click selects only the row clicked.
...
**`@kontsedal/olas-entities`.** A plugin (§13.1) that observes every cache write through `onWrite`, whatever its `source`, including a write another tab sent. It walks the data via per-entity `idOf` predicates, and maintains a normalized store plus a reverse index from entity id to the queries holding it. The store follows the data on screen, so it shows a pending guess while the guess is live. The settle re-walks the entry, a `'rollback'` and a `'commit'` alike, so the store holds settled data once every layer has settled. It exposes:
...
type AsyncState<T> = {
  ...
  firstValue: () => Promise<T>    // the data now if there is some, else the first success; waits while disabled
}
...
type RetryPolicy = false | number | ((attempt: number, error: unknown) => boolean) // false = never, as 0
...
type QuerySpec<Args extends unknown[], T> = {
  ...
  retryDelay?: RetryDelay    // default exponential, min(1000 * 2 ** attempt, 30_000); see §5.2
...
type Form<S extends FormSchema> = ReadSignal<FormValue<S>> & {
  ...
  readonly dirtyFields: ReadSignal<string[]>     // dotted paths of the dirty leaves; a structurally dirty array by its own path
  ...
  setErrors(errors: Record<string, ReadonlyArray<string>>): void // server errors by dotted path; a form or array path pins its topLevelErrors
...
Cache keys use a recursive tagged encoding: every primitive, array, object and bigint has its own type tag. Object properties are sorted; user data cannot impersonate a tag. The encoding hashes the value JSON round-trips a key to (§5.4): an `undefined` member is absent, `undefined` in an array and a non-finite number are `null`, and a Date is its ISO string. Cycles, functions, symbols, Map/Set and unsupported class instances throw. The hash string (`host.queries.hashKey`) is opaque, and its format is not a persistence protocol.

Expiry scheduling is shared. `scheduleExpiry` in `expiry-timer.ts` creates **no timer at all** for a non-finite delay, and walks a finite one in chunks against an absolute deadline. A delay above the platform's signed 32-bit limit therefore cannot overflow into an immediate fire. Every user-supplied duration goes through it: the staleness timer in `Entry` and `InfiniteEntry`, the gc timer in `ClientEntry` and `InfiniteClientEntry`, the `refetchInterval` chain, the retry backoff in `abortableSleep`, the windows of `debounced` and `throttled` (§9), and `suspend({ maxIdleTime })`. So `staleTime: Infinity` stays fresh until explicitly invalidated, and `gcTime: Infinity` retains a released entry for the life of the root. This applies to regular and infinite entries, initial hydration and streamed hydration.
...
// walks the regular and infinite entries, and serializes the server truth each
// one holds (a background refetch or a failed one included; the data under a
// live optimistic write) with its query id, key, data, the time the server
// said it and, for an infinite entry, pageParams
...
- The streaming hydrator serializes each batch that way, and `createStreamingTransform` writes a batch only outside every tag, attribute, comment, raw-text element and text node, and outside every element React hydrates (§16.1). React writes in fixed-size chunks, so a chunk can end inside a tag or an attribute value. `createStreamingHydrator({ nonce })` puts a CSP nonce on every tag it emits.
...
- `crossTabPlugin` ignores a message whose `msgId` is not a safe non-negative integer. It moves a peer's `msgId` cursor only for a message it applied, and a `msgId` far below the cursor restarts it, so a forged message cannot silence a peer. It reports a message it cannot apply through `onWarn`, and never lets it throw out of the channel's handler. Its `validate(queryId, data)` option lets a tab reject a payload shape it did not expect.
