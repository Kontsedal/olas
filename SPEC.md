...
**`write` supersedes an in-flight fetch; `setData` does not.** A canonical write is newer *by definition*: the caller is folding in something the server has already said, while an outstanding request was issued before that happened and will answer with the state from before it. So `write` aborts and supersedes that fetch itself — no `cancel(...)` at the call site, and the write stands. An optimistic `setData` gets the opposite rule, because it is a *guess*, and a server response is entitled to overrule a guess; that is why the cancel-before-`setData` step in §5.5 remains the caller's to make. The asymmetry is the reason the two methods are separate rather than one method with an options bag, and it is expressible here precisely because they *are* separate.

**A canonical write also rebases live optimistic snapshots** onto the written value, exactly as a successful fetch does and for the same reason: a later rollback must restore server truth, not a baseline captured before that truth arrived. This is not optional once `write` supersedes — the fetch it aborts is the one that used to do the rebasing, so without it a `write` landing mid-mutation is undone by that mutation's rollback, and undone to a value older than the one the write superseded.

**`write` supersedes only if it leaves the entry holding data** — asked *after* the updater runs, not before. The state to avoid is `status: 'success'` over `undefined` with nothing in flight, which nothing will refetch until `staleTime` lapses (a write flips an idle/pending entry to `success` whatever it is handed). That state is reached exactly when a write both supersedes the fetch and leaves `data` undefined, so testing the post-write value is the precise guard.

The order is load-bearing. Asking beforehand ("did this entry already hold data?") answers a worse question: it declines to supersede a full-body write onto an entry whose first load is still outstanding, which is the commonest shape of the bug — open a thing, act on it, and the push carrying the result is undone by the body read issued before the action. Nothing is stranded there, because the write itself supplies the value the fetch would have. What genuinely must not supersede is the merge that cannot patch what is not there (`prev ? fn(prev) : prev`, the usual shape over a possibly-absent key): it leaves `undefined`, so the in-flight fetch is still what will produce the first value and is left alone to do it. `query.cancel(...)` stays unconditional: it is the caller saying they know which case they are in.
