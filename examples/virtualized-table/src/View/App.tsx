      ...
      <div className="flex h-screen min-h-0 w-full flex-col gap-3 p-4">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="inline-flex h-9 w-9 items-center justify-center rounded-[var(--radius-surface)] bg-(--color-accent) text-(--color-accent-fg)">
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <input
        ...
        className="w-64 rounded-[var(--radius-control)] border border-(--color-border) bg-(--color-bg-elev) px-2 py-1 text-[length:var(--text-body)] outline-none focus:border-(--color-accent) focus:outline-2 focus:outline-offset-1 focus:outline-(--color-accent)"
      />
    </div>
  )
}
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-[var(--radius-surface)] border border-(--color-accent) bg-(--color-accent-soft) px-3 py-2 text-[length:var(--text-body)]">
      <span className="font-medium text-(--color-fg)">
        ...
        <span className="ml-2 text-[length:var(--text-meta)] font-normal text-(--color-fg-mute)">
      ...
      <div className="ml-auto flex flex-wrap items-center gap-2">
        {STATUS_OPTIONS.map((s) => (
          <button
            ...
            className="rounded-[var(--radius-control)] border border-(--color-border) bg-(--color-bg-elev) px-2 py-1 text-[length:var(--text-meta)] font-medium text-(--color-fg) hover:border-(--color-border-control) disabled:opacity-50"
          >
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded-[var(--radius-control)] border border-(--color-border) bg-(--color-bg-elev) px-2 py-1 text-[length:var(--text-meta)] text-(--color-fg-mute) hover:text-(--color-fg)"
        >
          <X className="size-3" />
        </button>
      </div>
...
function Legend(): ReactElement {
  return (
    <footer className="flex flex-wrap items-center gap-2 text-[length:var(--text-chrome)] text-(--color-fg-mute)">
