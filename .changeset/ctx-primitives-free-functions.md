...
**Breaking.** The lifetime-owned primitives take `ctx` as their first argument instead of hanging off it, and the query engine is now explicit on `createRoot`. A controllers-only bundle drops from 19.9 KB gzipped to 8.1 KB for consumers of the published package.
...
**Against the published `dist/`** — what a consumer actually gets: controllers only **8.1 KB**, was 19.9. The `QueryClient` exclusion survives the build intact.
