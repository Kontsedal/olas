...
A `fakeAsyncState` given an `error` now has status `'error'`, and its `firstValue()` rejects with it. A `'pending'` status reads as fetching, and as loading while there is no data. With no data, `firstValue()` stays pending, as a real subscription waits, instead of resolving `undefined`.
